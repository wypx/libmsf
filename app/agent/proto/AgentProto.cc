...
void AgentProto::debugBhs(const Agent::AgentBhs &bhs) {
  LOG(DEBUG) << "Bhs:";
  LOG(DEBUG) << "magic: " << std::hex << magic(bhs);
  LOG(DEBUG) << "version: " << (uint32_t)version(bhs);
  LOG(DEBUG) << "encrypt: " << (uint32_t)encrypt(bhs);
  LOG(DEBUG) << "checkSum: " << std::hex << checkSum(bhs);

  LOG(DEBUG) << "srcid: " << srcId(bhs);
  LOG(DEBUG) << "dstid: " << dstId(bhs);
  LOG(DEBUG) << "sessno: " << sessNo(bhs);
  LOG(DEBUG) << "retCode: " << retCode(bhs);

  LOG(DEBUG) << "command: " << std::hex << command(bhs);
  LOG(DEBUG) << "opcode: " << opCode(bhs);
  LOG(DEBUG) << "pdulen: " << pduLen(bhs);

  LOG(DEBUG) << "command: " << std::hex << bhs.verify();
  LOG(DEBUG) << "opcode: " << std::hex << bhs.router();
  LOG(DEBUG) << "pdulen: " << std::hex << bhs.command();
}
