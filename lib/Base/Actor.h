...
*
**************************************************************************/
#ifndef BASE_ACTOR_ACTOR_H_
#define BASE_ACTOR_ACTOR_H_
...
class ClassHandle {
  public:
      ClassHandle()
      {
        std::cout << "ClassHandle construct" << std::endl;
      }
      virtual ~ClassHandle() {};
...
class ClassHandleFactory
{
  ...
  public:
      ...
      {
          // std::cout << "static ClassHandleFactory* Instance() 1" << " : " << ppppp << std::endl;
          if (nullptr == ppppp)
          {
      ...
      bool Regist(const uint32_t cmd, const CreateClassFunc & func)
      {
          ...
          std::lock_guard<std::mutex> lock(mutex_);
          // bool isReg = registerFunctions_.insert(std::make_pair(std::string(typeName), func)).second;
  ...
  private:
      ...
      std::unordered_map<uint32_t, std::function<ClassHandle*(TArgs &&...)>> registerFunctions_;
...
public:
    struct Register
    {
        Register()
        {
            ...
            #ifdef __GNUC__
            demangleName = abi::__cxa_demangle(typeid(T).name(), nullptr, nullptr, nullptr);
            #else
            ...
            std::cout << "DynamicCreator Register typeName1: " << typeName << std::endl;
       
            std::cout << "DynamicCreator Register cmd: " << ++cmd << std::endl;
            ClassHandleFactory<TArgs...>::Instance()->Regist(cmd, CreateObject);
            // std::cout << "DynamicCreator Register typeName2: " << typeName << std::endl;
        }
...
template<typename T, typename ...TArgs>
typename DynamicCreator<T, TArgs...>::Register DynamicCreator<T, TArgs...>::register_;
...
class Step: public ClassHandle, public DynamicCreator<Step, std::string &, int>
{
  public:
...
class Worker
{
  public:
      ...
      {
          ClassHandle* p = 
            ClassHandleFactory<TArgs...>::Instance()->Create(cmd, std::forward<TArgs>(args)...);
          if (p == nullptr) {
      ...
      // {
      //     Cmd* cmd = dynamic_cast<Cmd*>(ClassHandleFactory<TArgs...>::Instance()->Create(c,std::forward<TArgs>(args)...));
      //     if (nullptr == cmd)
      //     {
