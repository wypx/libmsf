...
#ifndef USTEVENT_OBJ_POOL_H
#define USTEVENT_OBJ_POOL_H
...
public:
  ...
  uint64_t malloc_succeed_cnt() {
    return malloc_succeed_cnt_;
  }
  uint64_t malloc_failed_cnt() {
    return malloc_failed_cnt_;
  }
  uint64_t free_cnt() {
    return free_cnt_;
  }

  int32_t lowest_free_cnt() {
    return lowest_free_cnt_;
  }
 private:
  int GetIndex(void *p) {
