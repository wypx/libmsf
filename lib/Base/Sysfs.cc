...
int sysfs_init(void)
{
    ...
    LOG(INFO) << "sysfs_path=" << sysfs_path;
			     ...
			     const char *subsystem, const char *driver)
{
    ...
    LOG(INFO) << "kernel_number=" << dev->kernel_number;
}
...
int sysfs_resolve_link(char *devpath, size_t size)
{
    ...
    if (len <= 0)
        return -1;
    link_target[len] = '\0';
    LOG(INFO) << "path link " << devpath << " points to " << link_target;
    ...
    LOG(INFO) << "after moving back " << devpath;
...
struct sysfs_device *sysfs_device_get(const char *devpath)
{
    ...
    list_for_each_entry(dev_loop, &dev_list, node) {
        if (strcmp(dev_loop->devpath, devpath_real) == 0) {
            LOG(INFO) << "found in cache " << dev_loop->devpath;
            return dev_loop;
        }
    }
    ...
    /* it is a new device */
    LOG(INFO) << "new uncached device " << devpath_real;
...
struct sysfs_device *sysfs_device_get_parent(struct sysfs_device *dev)
{
    ...
    if (strncmp(parent_devpath, "/class", 6) == 0) {
        ...
        if (pos == &parent_devpath[6] || pos == parent_devpath) {
            LOG(INFO) << "/class top level, look for device link";
            goto device_link;
        }
    }
    if (strcmp(parent_devpath, "/block") == 0) {
        LOG(INFO) << "/block top level, look for device link";
        goto device_link;
    }
...
char *sysfs_attr_get_value(const char *devpath, const char *attr_name)
{
    ...
    if (S_ISLNK(statbuf.st_mode)) {
        ...
        if (len > 0) {
            ...
            pos = strrchr(link_target, '/');
            if (pos != NULL) {
                LOG_ERROR << "cache '" << path_full << "' with link value " << value;
    ...
    if (fd < 0) {
        LOG_ERROR << "attribute '" << path_full << "' can not be opened";
        goto out;
    }
...
char *sysfs_get_value(const char *id, char *subsys, char *param)
{
    ...
    if (!sysfs_lookup_devpath_by_subsys_id(devpath, sizeof(devpath),
                            subsys, id)) {
        LOG_ERROR << "Could not lookup devpath for "
                << subsys <<" " << id;
        return NULL;
    }
    ...
    if (!sysfs_value) {
        LOG_ERROR << "Could not read attr " << param << "on path " << devpath;
        return NULL;
    }
        ...
        char *write_buf, ssize_t buf_size)
{
    ...
    if (!sysfs_lookup_devpath_by_subsys_id(devpath, sizeof(devpath),
                            subsys, id)) {
        LOG_ERROR << "Could not lookup devpath for " << subsys << " " << id;
        return EIO;
    }
    ...
    if (lstat(path_full, &statbuf)) {
        LOG_ERROR << "Could not stat " << path_full;
        return errno;
    }

    if ((statbuf.st_mode & S_IWUSR) == 0) {
        LOG_ERROR << "Could not write to " << path_full << ". Invalid permissions.";
        return EACCES;
    }

    fd = open(path_full, O_WRONLY);
    if (fd < 0) {
        LOG_ERROR << "Could not open " << path_full << " err " << errno;
        return errno;
    }
