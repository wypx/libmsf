...
bool SetDcsp(const int fd, const uint32_t family, const uint32_t dscp) {
  ...
  if (dscp > 63) {
    LOG_WARN << "Qos dscp muts less than 63";
    return false;
  }
  val = dscp << 2;
  switch (family) {
...
int BindDevice(const int fd, const char *ifname) {
  if (fd >= 0 && ifname && ifname[0]) {
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr)) <
        0) {
      if (errno == EPERM) {
        LOG(ERROR) << "You must obtain superuser privileges to bind a socket to "
                     "device";
      } else {
...
int CreateUnixServer(const std::string &srvPath, const uint32_t backlog,
  ...
  if (chmod((char *)srvPath.c_str(), mode) == -1) {
    LOG_WARN << "chmod access failed for " << srvPath;
  }
...
int ConnectUnixServer(const std::string &srvPath, const std::string &cliPath) {
  ...
  if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    LOG(ERROR) << "Bind failed for fd: " << fd;
    close(fd);
    return -1;
  }
...
bool UpdateTimerFd(const int fd) {
  ...
  if (clock_gettime(CLOCK_MONOTONIC, &start) == -1) {
    LOG_FATAL << "failed to get clock realtime";
    return false;
  }
  ...
  if (timerfd_settime(fd, TFD_TIMER_ABSTIME, &new_value, NULL) == -1) {
    LOG_FATAL << "failed to set clock realtime";
    return false;
  }
  return true;
}
