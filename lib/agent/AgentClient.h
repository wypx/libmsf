...
typedef std::function<void (const char *data, const uint32_t len, const uint32_t cmd)> AgentCb;
...
struct AgentCmd
{
    ...
    AgentCmd(const AgentCb & cb = AgentCb())
    {
        doneCb_ = std::move(cb);
    }
    ~AgentCmd() { }
...
{
    public:
        explicit AgentClient(const std::string & host = "127.0.0.1", const uint16_t port = 8888);
        explicit AgentClient(const std::string & srvUnixPath, const std::string & cliUnixPath);
        ~AgentClient();
    ...
    private:
        ...
        std::map<uint32_t, struct AgentCmd*> ackCmdMap_;/* queue of ack cmd to handle*/
