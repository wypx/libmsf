...
int thread_pin_to_cpu(uint32_t cpu_id) 
{
    ...
    if (rc < 0) {
        // MSF_ERROR << "set thread(%u) affinity failed.", cpu_id);
    }
...
int process_pin_to_cpu(uint32_t cpu_id)
{
    ...
    if (cpuset_setaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, 
        ...
        MSF_ERROR << "Could not set CPU affinity, continuing.";
    }
...
#define MSF_SET_SPEC_PROC_PRIORITY(priority)    msf_set_priority(PRIO_PROCESS, getpid(), priority)
#define MSF_SET_GROUP_PROC_PRIORITY(priority)   msf_set_priority(PRIO_PGRP, getpid(), priority)
#define MSF_SET_USER_PROC_PRIORITY(priority)    msf_set_priority(PRIO_USER, getpid(), priority)
