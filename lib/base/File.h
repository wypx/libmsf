    ...
    public:
        ...
        bool CreateFileMap(struct MSF_FILEMAP *fm)
        {
            ...
            if (ftruncate(fm->fd, fm->size) == -1) {
                MSF_ERROR << "ftruncate file failed:" << fm->name;
            }
       ...
       /*  stat系统调用系列包括了fstat,stat和lstat x*/
        ...
        int FStatFile(int fd, struct stat* st)
        {
            return fstat(fd, st);
        }
        ...
        inline bool is_executable() const { return ((st.st_mode & S_IXUSR) == S_IXUSR); }
        ...
        bool is_writable() const 
        {
            ...
            if ((st.st_mode & S_IXUSR && st.st_uid == EUID) ||
                (st.st_mode & S_IXGRP && st.st_gid == EGID) ||
                (st.st_mode & S_IXOTH)) {
                return true;
            }
            return false;
        ...
        inline int file_fs_size() const { return std::max(st.st_size, st.st_blocks * 512); }
