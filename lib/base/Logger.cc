...
static const std::string g_LogLevel[] = { "DEBUG", "INFO", "WARN", "ERROR", "FATAL"};

static const std::string g_logcolor[] = {
    COLOR_GRAY, COLOR_GREEN, COLOR_YELLOW, 
    COLOR_BLUE, COLOR_RED, COLOR_PURPLE, 
    COLOR_BROWN, COLOR_BLUE, COLOR_CYAN, 
...
LogStream::~LogStream() 
{
    ...
    fmtStr  << COLOR_L_CYAN << " [pid:" << getpid() 
            << ",tid:" << gettid() << "]"
            << COLOR_NONE << std::endl;
...
LogStream::LogStream(enum LogLevel level, const char *func, 
                            const char *file, const int line)
{
    // std::cout << "MSF_LogStream is do start." << std::endl;
    fmtStr  << GetLogColor(level)
            << "[" << CurrTimeToString() << "]"
...
void Logger::setLogLevel(enum LogLevel level)
{
    iLogLevel = level;
}
...
{ 
    ...
    // std::cout << "MSF_LOG LogWriteFile flag: " <<  bLogFile << ":" << iLogFd << std::endl;
...
std::shared_ptr<LogStream> Logger::LogWrite(enum LogLevel level, const char *func, 
                        const char *file, const int line)
{
    //stLogLock.lock();
    ...
    logFormat->registCb(std::bind(&Logger::writeFile, this, 
                        std::placeholders::_1,
                        std::placeholders::_2));
    return logFormat;
}
