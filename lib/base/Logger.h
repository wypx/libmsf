...
class LogStream : public std::enable_shared_from_this<LogStream>
{
    public:
        ...
        explicit LogStream(enum LogLevel level, const char *func, 
                                const char *file, const int line) /*noexcept*/;
        virtual ~LogStream();

        virtual void registCb(pfnLogWriteCb wcb)
        {
            writeCb = std::move(wcb);
        }
...
#define MSF_INFO  Logger::getLogger().LogWrite(LEVEL_INFO, MSF_FUNC_FILE_LINE)->stream()
#define MSF_DEBUG Logger::getLogger().LogWrite(LEVEL_DEBUG, MSF_FUNC_FILE_LINE)->stream()
#define MSF_WARN  Logger::getLogger().LogWrite(LEVEL_WARN, MSF_FUNC_FILE_LINE)->stream()
#define MSF_ERROR Logger::getLogger().LogWrite(LEVEL_ERROR, MSF_FUNC_FILE_LINE)->stream()
#define MSF_FATAL Logger::getLogger().LogWrite(LEVEL_FATAL, MSF_FUNC_FILE_LINE)->stream()
