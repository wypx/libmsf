...
struct MemSlab {
    ...
    MemSlab(enum MemBlkIdx blkIdx, uint32_t minBlkNR,
...
class MemPool : Noncopyable {
    public:
        ...
        explicit MemPool(const int numaId, 
                const uint32_t blkAlign,
                bool safeMt = true)
                : _numaId(numaId),
                 _safeMt(safeMt),
                 _blkAlign(blkAlign),
    ...
    private:
        ...
        void addMemSlab(enum MemBlkIdx blkIdx, uint32_t minBlkNR,
                uint32_t maxBlkNR, uint32_t growBlkNR);
        void addDefMemSlab();
};
