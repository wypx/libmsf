...
int sysfs_init(void)
{
    ...
    MSF_INFO << "sysfs_path=" << sysfs_path;
			     ...
			     const char *subsystem, const char *driver)
{
    ...
    MSF_INFO << "kernel_number=" << dev->kernel_number;
}
...
int sysfs_resolve_link(char *devpath, size_t size)
{
    ...
    if (len <= 0)
        return -1;
    link_target[len] = '\0';
    MSF_INFO << "path link " << devpath << " points to " << link_target;
    ...
    MSF_INFO << "after moving back " << devpath;
...
struct sysfs_device *sysfs_device_get(const char *devpath)
{
    ...
    list_for_each_entry(dev_loop, &dev_list, node) {
        if (strcmp(dev_loop->devpath, devpath_real) == 0) {
            MSF_INFO << "found in cache " << dev_loop->devpath;
            return dev_loop;
        }
    }
    ...
    /* it is a new device */
    MSF_INFO << "new uncached device " << devpath_real;
    dev = (struct sysfs_device *)malloc(sizeof(struct sysfs_device));
    if (dev == NULL)
        return NULL;
...
struct sysfs_device *sysfs_device_get_parent(struct sysfs_device *dev)
{
    ...
    if (strncmp(parent_devpath, "/class", 6) == 0) {
        ...
        if (pos == &parent_devpath[6] || pos == parent_devpath) {
            MSF_INFO << "/class top level, look for device link";
            goto device_link;
        }
    }
    if (strcmp(parent_devpath, "/block") == 0) {
        MSF_INFO << "/block top level, look for device link";
        goto device_link;
    }
...
char *sysfs_attr_get_value(const char *devpath, const char *attr_name)
{
    ...
    if (S_ISLNK(statbuf.st_mode)) {
        ...
        if (len > 0) {
            ...
            pos = strrchr(link_target, '/');
            if (pos != NULL) {
                MSF_ERROR << "cache '" << path_full << "' with link value " << value;
    ...
    if (fd < 0) {
        MSF_ERROR << "attribute '" << path_full << "' can not be opened";
        goto out;
    }
...
char *sysfs_get_value(const char *id, char *subsys, char *param)
{
    ...
    if (!sysfs_lookup_devpath_by_subsys_id(devpath, sizeof(devpath),
                            subsys, id)) {
        MSF_ERROR << "Could not lookup devpath for "
                << subsys <<" " << id;
        return NULL;
    }
    ...
    if (!sysfs_value) {
        MSF_ERROR << "Could not read attr " << param << "on path " << devpath;
        return NULL;
    }
        ...
        char *write_buf, ssize_t buf_size)
{
    ...
    if (!sysfs_lookup_devpath_by_subsys_id(devpath, sizeof(devpath),
                            subsys, id)) {
        MSF_ERROR << "Could not lookup devpath for " << subsys << " " << id;
        return EIO;
    }
    ...
    if (lstat(path_full, &statbuf)) {
        MSF_ERROR << "Could not stat " << path_full;
        return errno;
    }

    if ((statbuf.st_mode & S_IWUSR) == 0) {
        MSF_ERROR << "Could not write to " << path_full << ". Invalid permissions.";
        return EACCES;
    }

    fd = open(path_full, O_WRONLY);
    if (fd < 0) {
        MSF_ERROR << "Could not open " << path_full << " err " << errno;
        return errno;
    }
...
char *sysfs_get_uevent_field(const char *path, const char *field)
{
    ...
    uevent_path = (char *)calloc(1, PATH_MAX);
    if (!uevent_path)
        return NULL;
