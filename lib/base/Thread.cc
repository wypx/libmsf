...
int pthreadSpawn(pthread_t *tid, void* (*pfn)(void *), void *arg)
{
    ...
    if (pthread_create(tid, &thread_attr, pfn, arg) < 0) {
        MSF_ERROR << "Do pthread_create errno: " << errno;
        return -1;
    }
...
void* Thread::threadLoop(ThreadFunc initFunc) 
{
    ...
    MSF_INFO << "Thread name: " << CurrentThread::t_threadName << " name:" << _name;
    ...
    try {
        ...
        MSF_FATAL  << "exception caught in ThreadPool " << _name.c_str()
                    << ",reason: " << ex.what()
        ...
        CurrentThread::t_threadName = "crashed";
        MSF_FATAL   << "unknown exception caught in ThreadPool " << _name.c_str();
        throw; // rethrow
    }
    return nullptr;
}
...
void Thread::start(const ThreadFunc & initFunc)
{
    ...
    try {
        ...
        CurrentThread::t_threadName = "crashed";
        MSF_FATAL  << "Fail to create thread ThreadPool " << _name.c_str()
                    << ",reason: " << ex.what()
                    << "stack trace: " << ex.stackTrace();
        _started = false;
    } catch (const std::exception& ex) {
        ...
        CurrentThread::t_threadName = "crashed";
        MSF_FATAL   << "Fail to create thread ThreadPool, unkown exception " << _name.c_str();
        _started = false;
        throw; // rethrow
    }
}
...
{
    if (!_started) {
        MSF_ERROR << "Thread already started";
        return;
    }

    if (!_thread.joinable()) {
        MSF_WARN << "Thread not allow to join";
        return;
    }
    _thread.join();
}
