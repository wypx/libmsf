...
//         if (result != 0)
//             throwex SystemException("Failed to initialize a condition
// variable!", result);
// #elif defined(_WIN32) || defined(_WIN64)
...
//         if (result != 0)
//             fatality(SystemException("Failed to destroy a condition
// variable!", result));
// #elif defined(_WIN32) || defined(_WIN64)
...
//         if (result != 0)
//             throwex SystemException("Failed to signal a condition variable!",
// result);
// #elif defined(_WIN32) || defined(_WIN64)
...
//         if (result != 0)
//             throwex SystemException("Failed to broadcast a condition
// variable!", result);
// #elif defined(_WIN32) || defined(_WIN64)
...
// #if defined(unix) || defined(__unix) || defined(__unix__) ||
// defined(__APPLE__)
//         int result = pthread_cond_wait(&_cond,
// (pthread_mutex_t*)cs.native());
//         if (result != 0)
//             throwex SystemException("Failed to waiting a condition
// variable!", result);
// #elif defined(_WIN32) || defined(_WIN64)
//         if (!SleepConditionVariableCS(&_cond, (CRITICAL_SECTION*)cs.native(),
// INFINITE))
//             throwex SystemException("Failed to waiting a condition
// variable!");
// #endif
...
//         int result = pthread_cond_timedwait(&_cond,
// (pthread_mutex_t*)cs.native(), &timeout);
//         if ((result != 0) && (result != ETIMEDOUT))
//             throwex SystemException("Failed to waiting a condition variable
// for the given timeout!", result);
//         return (result == 0);
// #elif defined(_WIN32) || defined(_WIN64)
//         if (!SleepConditionVariableCS(&_cond, (CRITICAL_SECTION*)cs.native(),
// std::max((DWORD)0, (DWORD)timespan.milliseconds())))
//         {
//             if (GetLastError() != ERROR_TIMEOUT)
//                 throwex SystemException("Failed to waiting a condition
// variable for the given timeout!");
//             return false;
//         }
//         return true;
// #endif
...
//     static_assert((StorageSize >= sizeof(Impl)),
...
// bool ConditionVariable::TryWaitFor(CriticalSection& cs, const Timespan&
// timespan) { return impl().TryWaitFor(cs, timespan); }
