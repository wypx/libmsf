...
// /*!
//     Condition variable is a synchronization primitive that enable threads to
// wait
//     until a particular condition occurs. Condition variables are user-mode
// objects
//     that cannot be shared across processes.

//     Condition variables enable threads to atomically release a lock and enter
// the
//     sleeping state. They can be used with critical sections. Condition
// variables
//     support operations that "notify one" or "notify all" waiting threads.
// After
//     a thread is woken, it re-acquires the lock it released when the thread
// entered
//     the sleeping state.
...
//     /*!
//         Unblocks one of the threads currently waiting for this condition. If
// no
//         threads are waiting, the function does nothing. If more than one, it
// is
...
//     /*!
//         Unblocks all threads currently waiting for this condition. If no
// threads
...
//     //! Try to wait for the given timespan until condition variable is
// notified
//     /*!
//         The execution of the current thread (which shall have locked critical
...
//         \param cs - Critical section (must be locked)
//         \param timespan - Timespan to wait for the condition variable
// notification
//         \return 'true' if the condition variable was successfully notified,
// 'false' if the timeout was occurred
//     */
//     bool TryWaitFor(CriticalSection& cs, const Timespan& timespan);
//     //! Try to wait for the given timespan until condition variable is
// notified using the given predicate
//     /*!
//         This method is equivalent to:
//         \code{.cpp}
...
//     */
//     template <typename TPredicate>
//     bool TryWaitFor(CriticalSection& cs, const Timespan& timespan, TPredicate
// predicate);

//     //! Try to wait until the given timestamp until condition variable is
// notified
//     /*!
//         The execution of the current thread (which shall have locked critical
// section)
//         is blocked either until notified or until timestamp, whichever
// happens first.

//         Will block until the given timestamp in the worst case.

//         \param cs - Critical section (must be locked)
//         \param timestamp - Timestamp to stop wait for the condition variable
// notification
//         \return 'true' if the condition variable was successfully notified,
// 'false' if the timeout was occurred
//     */
...
// /*! \example threads_condition_variable.cpp Condition variable
// synchronization primitive example */
