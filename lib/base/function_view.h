#ifndef LIB_FUNCTION_VIEW_H_
#define LIB_FUNCTION_VIEW_H_
...
class FunctionView<RetT(ArgT...)> final {
 public:
  ...
  FunctionView(F&& f)
      : call_(call_void_ptr<typename std::remove_reference<F>::type>) {
    func_.void_ptr = &f;
  }
  ...
  template <
      ...
      typename std::enable_if<std::is_function<typename std::remove_pointer<
          ...
          nullptr>
  FunctionView(F&& f)
      : call_(f ? call_fun_ptr<typename std::remove_pointer<F>::type>
                : nullptr) {
    func_.fun_ptr = reinterpret_cast<void (*)()>(f);
  }
  ...
  RetT operator()(ArgT... args) const {
    return call_(func_, std::forward<ArgT>(args)...);
  }
