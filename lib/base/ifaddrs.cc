...
int getifaddrs(struct ifaddrs **ifap) {
  ...
  DWORD size, res = 0;
  res = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, NULL, NULL,
                             &size);
  ...
  for (PIP_ADAPTER_ADDRESSES curr_addrs = adapter_addrs; curr_addrs != NULL;
    ...
    for (PIP_ADAPTER_UNICAST_ADDRESS unicast_addrs =
      ...
      out_list_curr->ifa_netmask =
          (struct sockaddr *)&out_list_curr->in_netmasks;
      out_list_curr->ifa_name = out_list_curr->ad_name;

      if (unicast_sockaddr->sa_family == AF_INET) {
        ULONG subnet_mask = 0;
        if (ConvertLengthToIpv4Mask(unicast_addrs->OnLinkPrefixLength,
                                    &subnet_mask)) {
          errno = ENODATA;
          ret = -1;
          goto out;
      ...
      sprintf_s(out_list_curr->ad_name, sizeof(out_list_curr->ad_name),
