...
enum MemBlkIdx MemPool::GetMemBlkIdx(const uint32_t size) {
  ...
  MSF_INFO << "Mempool blk idx: " << idx << " size: " << kMemBlkSizeArray[idx];
...
bool MemPool::Init() {
  if (blk_algin_ == 0) {
    ...
    MSF_ERROR << "Invalid alignment: " << blk_algin_;
    return false;
  }

  MSF_INFO << "Flag is:" << flags_;
  if (MSF_TEST_BITS(MP_FLAG_HUGE_PAGES_ALLOC, &flags_)) {
...
void MemPool::AddMemSlab(enum MemBlkIdx blkIdx, uint32_t minBlkNR,
  ...
  if (unlikely(slabs_.size() > max_slab_)) {
    MSF_ERROR << "Not support any more slab config: " << max_slab_;
    return;
  }
