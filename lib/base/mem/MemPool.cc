...
static const uint32_t _MemBlkSizeArray_[MEMBLK_MAX] = {32,
                                                       64,
                                                       128,
                                                       256,
                                                       512,
...
enum MemBlkIdx MemPool::getMemBlkIdx(const uint32_t size) {
  ...
  uint32_t idx = std::upper_bound(_MemBlkSizeArray_,
                                  _MemBlkSizeArray_ + MEMBLK_MAX, alignSize) -
                 _MemBlkSizeArray_;

  MSF_INFO << "Mempool blk idx: " << idx << " size: " << _MemBlkSizeArray_[idx];
...
bool MemPool::createSlab() {
  std::for_each(_slabs.begin(), _slabs.end(), [this](auto &slab) {
    ...
    for (uint32_t i = 0; i < slab._minBlkNR; ++i) {
      ...
      _slabBuffMap.insert(std::make_pair<uint64_t, uint64_t>(
          (uint64_t)blk._buffer, (uint64_t)std::addressof(slab)));
    }
    return true;
  });
  return true;
...
bool MemPool::init() {
  ...
  } else if (!isPowerOf2(_blkAlign) || !(_blkAlign % sizeof(void *) == 0)) {
...
void MemPool::free(const void *ptr) {
  ...
  MSF_INFO << "Slab addr: " << slab << " Memblk addr: " << blk
           << " Buff addr: " << ptr;
  slab->_freeBlkList.push_back(std::move(*blk));
