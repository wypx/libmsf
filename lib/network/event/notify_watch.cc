 ...
 * @internal
 */
void cleanup_tree(const void *nodep, const VISIT which, const int depth,
                  void *arg) {
 ...
 * @internal
 */
void empty_stats(const void *nodep, const VISIT which, const int depth,
                 void *arg) {
  if (which != endorder && which != leaf) return;
  watch *w = (watch *)nodep;
  w->hit_access = 0;
 ...
 * @internal
 */
void get_num(const void *nodep, const VISIT which, const int depth, void *arg) {
  if (which != endorder && which != leaf) return;
  ++(*((int *)arg));
}
...
int inotifytools_str_to_event_sep(char const *event, char sep) {
  ...
  while (event1 && event1[0]) {
    if (event2) {
      len = event2 - event1;
      niceassert(len < eventstr_size, "malformed event string (very long)");
    } else {
      len = strlen(event1);
    }
    if (len > eventstr_size - 1) len = eventstr_size - 1;
...
void inotifytools_replace_filename(char const *oldname, char const *newname) {
  ...
  rbwalk(tree_filename, (const void *)replace_filename, (void *)&data);
}
...
int inotifytools_watch_files(char const *filenames[], int events) {
  ...
  for (i = 0; filenames[i]; ++i) {
    ...
    if (!isdir(filenames[i]) || filenames[i][strlen(filenames[i]) - 1] == '/') {
...
#define RETURN(A)                                                  \
  {                                                                \
    if (regex) {                                                   \
      ...
      if (0 == regexec(regex, match_name_string, 0, 0, 0)) {       \
        if (!invert_regexp) longjmp(jmp, 0);                       \
      } else {                                                     \
        if (invert_regexp) longjmp(jmp, 0);                        \
      }                                                            \
    }                                                              \
  ...
  if (first_byte != 0 &&
    ...
    if (first_byte == bytes) {
      ...
      niceassert((long)((char *)&event[0] + sizeof(struct inotify_event) +
  ...
  } while (!rc && bytes_to_read < sizeof(struct inotify_event) * num_events);
    ...
    if ((0 != strcmp(ent->d_name, ".")) && (0 != strcmp(ent->d_name, ".."))) {
      ...
      if (-1 == lstat64(next_file, &my_stat)) {
        ...
        for (exclude_entry = exclude_list;
             exclude_entry && *exclude_entry && !no_watch; ++exclude_entry) {
...
unsigned int *stat_ptr(watch *w, int event) {
  if (IN_ACCESS == event) return &w->hit_access;
  if (IN_MODIFY == event) return &w->hit_modify;
  if (IN_ATTRIB == event) return &w->hit_attrib;
  if (IN_CLOSE_WRITE == event) return &w->hit_close_write;
  if (IN_CLOSE_NOWRITE == event) return &w->hit_close_nowrite;
  if (IN_OPEN == event) return &w->hit_open;
  if (IN_MOVED_FROM == event) return &w->hit_moved_from;
  if (IN_MOVED_TO == event) return &w->hit_moved_to;
  if (IN_CREATE == event) return &w->hit_create;
  if (IN_DELETE == event) return &w->hit_delete;
  if (IN_DELETE_SELF == event) return &w->hit_delete_self;
  if (IN_UNMOUNT == event) return &w->hit_unmount;
  if (IN_MOVE_SELF == event) return &w->hit_move_self;
  if (0 == event) return &w->hit_total;
  return 0;
}
...
int inotifytools_get_stat_total(int event) {
  if (!collect_stats) return -1;
  if (IN_ACCESS == event) return num_access;
  if (IN_MODIFY == event) return num_modify;
  if (IN_ATTRIB == event) return num_attrib;
  if (IN_CLOSE_WRITE == event) return num_close_write;
  if (IN_CLOSE_NOWRITE == event) return num_close_nowrite;
  if (IN_OPEN == event) return num_open;
  if (IN_MOVED_FROM == event) return num_moved_from;
  if (IN_MOVED_TO == event) return num_moved_to;
  if (IN_CREATE == event) return num_create;
  if (IN_DELETE == event) return num_delete;
  if (IN_DELETE_SELF == event) return num_delete_self;
  if (IN_UNMOUNT == event) return num_unmount;
  if (IN_MOVE_SELF == event) return num_move_self;

  if (0 == event) return num_total;
...
int inotifytools_get_stat_by_filename(char const *filename, int event) {
  return inotifytools_get_stat_by_wd(inotifytools_wd_from_filename(filename),
                                     event);
}
 ...
 */
int inotifytools_error() { return error; }
 ...
 *            incorrect results.
 */
void inotifytools_set_printf_timefmt(char *fmt) { timefmt = fmt; }
 ...
 * ignored.
 */
static int do_ignore_events_by_regex(char const *pattern, int flags,
                                     int invert) {
  if (!pattern) {
    if (regex) {
      regfree(regex);
