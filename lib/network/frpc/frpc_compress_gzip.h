 ...
 *
 **************************************************************************/
#ifndef FRPC_COMPRESS_GZIP_H_
#define FRPC_COMPRESS_GZIP_H_
...
/*
static int protobuf_stream_compress(const ProtobufIDLMessage* msg, char *buf,
size_t buflen,
                                                                        int
compress_type)
{
...
static int protobuf_stream_decompress(const char *buf, size_t buflen,
ProtobufIDLMessage* msg, size_t msglen,
                                                                          int
compress_type)
{
...
static int CommonCompress(const char *msg, size_t msglen, char *buf,
                          size_t buflen, int option_format) {
  if (!msg) return 0;
 ...
 */
static inline int GzipCompress(const char *msg, size_t msglen, char *buf,
                               size_t buflen) {
 ...
 * 		>0: byte count of compressed data
 */
static inline int ZlibCompress(const char *msg, size_t msglen, char *buf,
                               size_t buflen) {
  return CommonCompress(msg, msglen, buf, buflen, OPTION_FORMAT_ZLIB);
}

static constexpr unsigned char dummy_head[2] = {
    0xB + 0x8 * 0x10, (((0x8 + 0x7 * 0x10) * 0x100 + 30) / 31 * 31) & 0xFF, };
 ...
 * 		>0: byte count of compressed data
 */
static int CommonDecompress(const char *buf, size_t buflen, char *msg,
                            size_t msglen) {
  int err;
  ...
  while (d_stream.total_out < msglen && d_stream.total_in < buflen) {
    ...
    if (err != Z_OK) {
      ...
      if (inflate(&d_stream, Z_NO_FLUSH) != Z_OK) return -1;
    }
  }
...
static int CommonCompressIOVec(Buffer *src, Buffer *dst, int option_format) {
  z_stream c_stream;
  int err;
  ...
  while (c_stream.total_in < buflen) {
    if (c_stream.avail_in == 0) {
      if ((c_stream.avail_in = (uInt)src->fetch(&in)) == 0) return -1;

      c_stream.next_in = static_cast<Bytef *>(const_cast<void *>(in));
    }

    if (c_stream.avail_out == 0) {
      if (dst->acquire(&out, &out_len) == false) return -1;
...
/*
 * compress serialized Buffer src(Source) into Buffer dst(Sink).
 * ret: -1: failed
 * 		>0: byte count of compressed data
 */
static int GzipCompressIOVec(Buffer *src, Buffer *dst) {
 ...
 * 		>0: byte count of compressed data
 */
static int ZlibCompressIOVec(Buffer *src, Buffer *dst) {
 ...
 * 		>0: byte count of compressed data
 */
static int CommonDecompressIOVec(Buffer *src, Buffer *dst) {
  int err;
  z_stream d_stream = {0}; /* decompression stream */
  size_t total_alloc = 0;
  const void *in;
  ...
  while (d_stream.total_in < buflen) {
    if (d_stream.avail_in == 0) {
      if ((d_stream.avail_in = (uInt)src->fetch(&in)) == 0) return -1;
