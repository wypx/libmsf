...
void Shutdown(const int fd, int how) { ::shutdown(fd, how); }
 ...
 * the target file, not any intermediate symlinks along the path)*/
...
int OpenCloeExec(const char *pathname, bool symlinks, int flags, mode_t mode) {
...
int OpenDevNull(bool rw) {
  int flags = rw ? O_RDWR : O_RDONLY;
#if defined(_WIN32_)
  return OpenCloeExec("nul", 0, flags, 0);
#else
  return OpenCloeExec("/dev/null", false, flags, 0);
#endif
}
...
bool SetKeepAlive(const int fd, bool on) {
  ...
  tcp_ka.onoff = 1;
  tcp_ka.keepalivetime = val * 1000;
  tcp_ka.keepaliveinterval = val / 3 * 1000;

  int result = ::WSAIoctl(fd_, SIO_KEEPALIVE_VALS, &tcp_ka, sizeof(tcp_ka),
...
// https://www.cnblogs.com/orange1438/p/4613460.html
ssize_t ReadDiscard(int fd, char *buf, size_t sz, int family, int so_type) {
...
bool SetTcpDeferAccept(const int fd) {
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_ACCEPTFILTER, &afa, sizeof(afa)) < 0) {
    if (errno != ENOENT) {
      // LOG(ERROR) << "SBss can't set accept-filter '", s->bsd_accept_filter,
      // "':", strerror(errno));
    }
  }
#endif
...
char *GetRoutingDomain(int fd) {
  ...
  if (getsockopt(fd, SOL_SOCKET, SO_RTABLE, &rtable, &len) == -1) {
    LOG(ERROR) << "failed to get routing domain for fd <<" << fd << ":"
               << strerror(errno);
    return NULL;
  }
...
int SetRoutingDomain(int fd, const char *name) {
  ...
  if (name == NULL) return 0; /* default table */
  ...
  if (::setsockopt(fd, SOL_SOCKET, SO_RTABLE, &rtable, sizeof(rtable)) == -1) {
    error("Failed to set routing domain %d on fd %d: %s", rtable, fd,
          strerror(errno));
    return -1;
  }
  return 0;
#else /* defined(__OpenBSD__) */
  LOG(ERROR) << "setting routing domain is not supported on this platform";
  return -1;
#endif
}
