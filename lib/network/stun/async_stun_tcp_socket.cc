...
AsyncStunTCPSocket::AsyncStunTCPSocket(AsyncSocket* socket, bool listen)
...
int AsyncStunTCPSocket::Send(const void* pv, size_t cb,
  ...
  if (cb != expected_pkt_len) return -1;
...
void AsyncStunTCPSocket::ProcessInput(char* data, size_t* len) {
  ...
  while (true) {
    ...
    if (*len < kPacketLenOffset + kPacketLenSize) return;
    ...
    SignalReadPacket(this, data, expected_pkt_len, remote_addr, TimeMicros());
...
void AsyncStunTCPSocket::HandleIncomingConnection(AsyncSocket* socket) {
