...
class DtlsTransport : public DtlsTransportInternal {
 public:
  ...
  bool SetRemoteFingerprint(const std::string& digest_alg,
                            const uint8_t* digest, size_t digest_len) override;
  ...
  bool ExportKeyingMaterial(const std::string& label, const uint8_t* context,
 ...
 private:
  ...
  void OnReadPacket(rtc::PacketTransportInternal* transport, const char* data,
                    size_t size, const int64_t& packet_time_us, int flags);
  ...
  StreamInterfaceChannel* downward_;  // Wrapper for ice_transport_, owned by
                                      // dtls_.
