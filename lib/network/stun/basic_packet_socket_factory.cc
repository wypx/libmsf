...
BasicPacketSocketFactory::BasicPacketSocketFactory(EventLoop* loop)
    : loop_(loop), socket_factory_(nullptr) {
  if (!socket_factory_) {
    internal_socket_factory_.reset(new PhysicalSocketServer());
    socket_factory_ = internal_socket_factory_.get();
  }
}

BasicPacketSocketFactory::BasicPacketSocketFactory(
    EventLoop* loop, SocketFactory* socket_factory)
    : loop_(loop), socket_factory_(socket_factory) {
  if (!socket_factory_) {
    internal_socket_factory_.reset(new PhysicalSocketServer());
    socket_factory_ = internal_socket_factory_.get();
  }
...
AsyncPacketSocket* BasicPacketSocketFactory::CreateUdpSocket(
    const SocketAddress& address, uint16_t min_port, uint16_t max_port) {
  ...
  if (BindSocket(socket, address, min_port, max_port) < 0) {
    LOG(ERROR) << "UDP bind failed with error " << socket->GetError();
    delete socket;
    return NULL;
  }
...
AsyncPacketSocket* BasicPacketSocketFactory::CreateServerTcpSocket(
  ...
  if (opts & PacketSocketFactory::OPT_TLS) {
    LOG(ERROR) << "TLS support currently is not available.";
    return NULL;
  }
  ...
  if (BindSocket(socket, local_address, min_port, max_port) < 0) {
    LOG(ERROR) << "TCP bind failed with error " << socket->GetError();
    delete socket;
    return NULL;
  }
...
AsyncPacketSocket* BasicPacketSocketFactory::CreateClientTcpSocket(
  ...
  if (BindSocket(socket, local_address, 0, 0) < 0) {
    ...
    if (local_address.IsAnyIP()) {
      LOG(WARNING) << "TCP bind failed with error " << socket->GetError()
  ...
  if (socket->Connect(remote_address) < 0) {
    LOG(ERROR) << "TCP connect failed with error " << socket->GetError();
    delete socket;
    return NULL;
  }
