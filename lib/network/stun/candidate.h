...
class Candidate {
 public:
  ...
  Candidate(int component, const std::string& protocol,
            const SocketAddress& address, uint32_t priority,
            const std::string& username, const std::string& password,
            const std::string& type, uint32_t generation,
            const std::string& foundation, uint16_t network_id = 0,
  ...
  // kNetworkCostMax indicates it should be used only as the last resort.
  void set_network_cost(uint16_t network_cost) {
    assert(network_cost <= kNetworkCostMax);
  ...
  uint32_t GetPriority(uint32_t type_preference, int network_adapter_preference,
