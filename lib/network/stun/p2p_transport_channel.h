...
#include "async_resolver_factory.h"
#include "candidate.h"
#include "candidate_pair_interface.h"
#include "ice_controller_factory_interface.h"
#include "ice_controller_interface.h"
#include "ice_transport_internal.h"
#include "p2p_constants.h"
#include "p2p_transport_channel_ice_field_trials.h"
#include "port_allocator.h"
#include "port_interface.h"
#include "regathering_controller.h"
...
class P2PTransportChannel : public IceTransportInternal {
 public:
  ...
  P2PTransportChannel(const std::string& transport_name, int component,
  ...
  IceTransportStandardState GetIceTransportState() const override;
  ...
  // static RTCError ValidateIceConfig(const IceConfig& config);

  // From TransportChannel:
  int SendPacket(const char* data, size_t len, const PacketOptions& options,
                 int flags) override;
  ...
  // Public for unit tests.
  ArrayView<Connection*> connections() const;
 ...
 private:
  ...
  IceTransportStandardState ComputeIceTransportState() const;
  ...
  AsyncResolverFactory* async_resolver_factory_;
  // Thread* loop_;
  EventLoop* loop_;
  bool incoming_only_;
  int error_;
  ...
  bool sort_dirty_;              // indicates whether
                                 // another sort is needed
                                 // right now
  ...
  IceTransportStandardState standardized_state_ =
      IceTransportStandardState::kNew;
  IceConfig config_;
  ...
  void OnCandidateResolved(AsyncResolverInterface* resolver);
