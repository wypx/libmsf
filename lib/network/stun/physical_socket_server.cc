...
#if defined(__linux__) && !defined(WEBRTC_MAC) && !defined(__native_client__)
...
int64_t GetSocketRecvTimestamp(int socket) { return -1; }
#endif
...
PhysicalSocket::PhysicalSocket(PhysicalSocketServer* ss, SOCKET s)
    : ss_(ss),
      s_(s),
      error_(0),
      state_((s == -1) ? CS_CLOSED : CS_CONNECTED)
      /*resolver_(nullptr)*/ {
  if (s_ != -1) {
...
SocketAddress PhysicalSocket::GetLocalAddress() const {
  ...
  if (result >= 0) {
    ...
    LOG(WARNING) << "GetLocalAddress: unable to get local addr, socket=" << s_;
  }
  return address;
}
...
int PhysicalSocket::Bind(const SocketAddress& bind_addr) {
  ...
  if (ss_->network_binder() && !bind_addr.IsAnyIP()) {
    ...
    if (result == NetworkBindingResult::SUCCESS) {
      ...
      LOG(INFO) << "Can't bind socket to network because "
      ...
      if (bind_addr.IsLoopbackIP()) {
        ...
        LOG(TRACE) << "Binding socket to loopback address "
                   << bind_addr.ipaddr().ToString()
                   << " failed; result: " << static_cast<int>(result);
      } else {
        LOG(WARNING) << "Binding socket to network address "
                     << bind_addr.ipaddr().ToString()
                     << " failed; result: " << static_cast<int>(result);
...
int PhysicalSocket::Connect(const SocketAddress& addr) {
  ...
  if (addr.IsUnresolvedIP()) {
    LOG(TRACE) << "Resolving addr in PhysicalSocket::Connect";
...
int PhysicalSocket::DoConnect(const SocketAddress& connect_addr) {
  if ((s_ == -1) && !Create(connect_addr.family(), SOCK_STREAM)) {
    return -1;
  }
  sockaddr_storage addr_storage;
...
AsyncSocket::ConnState PhysicalSocket::GetState() const { return state_; }
...
#if defined(__linux__) && !defined(WEBRTC_ANDROID)
...
int PhysicalSocket::SendTo(const void* buffer, size_t length,
...
int PhysicalSocket::RecvFrom(void* buffer, size_t length,
                             SocketAddress* out_addr, int64_t* timestamp) {
  sockaddr_storage addr_storage;
...
SOCKET PhysicalSocket::DoAccept(SOCKET socket, sockaddr* addr,
                                socklen_t* addrlen) {
...
int PhysicalSocket::DoSendTo(SOCKET socket, const char* buf, int len, int flags,
...
void PhysicalSocket::UpdateLastError() { SetError(LAST_SYSTEM_ERROR); }
...
void PhysicalSocket::EnableEvents(uint8_t events) { enabled_events_ |= events; }
  ...
  switch (opt) {
    case OPT_DONTFRAGMENT:
      ...
      *sopt = IP_DONTFRAGMENT;
      ...
      LOG(WARNING) << "Socket::OPT_DONTFRAGMENT not supported.";
      return -1;
#elif defined(__linux__)
      *slevel = IPPROTO_IP;
...
bool SocketDispatcher::Create(int type) { return Create(AF_INET, type); }
...
WSAEVENT SocketDispatcher::GetWSAEvent() { return WSA_INVALID_EVENT; }

SOCKET SocketDispatcher::GetSocket() { return s_; }

bool SocketDispatcher::CheckSignalClose() {
  if (!signal_close_) return false;

  char ch;
  if (recv(s_, &ch, 1, MSG_PEEK) > 0) return false;
...
int SocketDispatcher::GetDescriptor() { return s_; }
...
uint32_t SocketDispatcher::GetRequestedEvents() { return enabled_events(); }
...
void SocketDispatcher::OnEvent(uint32_t ff, int err) {
  ...
  if (((ff & DE_CONNECT) != 0) && (id_ == cache_id)) {
    if (ff != DE_CONNECT) LOG(TRACE) << "Signalled with DE_CONNECT: " << ff;
...
class EventDispatcher : public Dispatcher {
 public:
  EventDispatcher(PhysicalSocketServer* ss) : ss_(ss), fSignaled_(false) {
    if (pipe(afd_) < 0) LOG(ERROR) << "pipe failed";
    ss_->Add(this);
  }
...
static uint32_t FlagsToEvents(uint32_t events) {
  ...
  if (events & DE_READ) ffFD |= FD_READ;
  if (events & DE_WRITE) ffFD |= FD_WRITE;
  if (events & DE_CONNECT) ffFD |= FD_CONNECT;
  if (events & DE_ACCEPT) ffFD |= FD_ACCEPT;
  return ffFD;
}
...
class EventDispatcher : public Dispatcher {
 public:
  ...
  virtual void Signal() {
    if (hev_ != nullptr) WSASetEvent(hev_);
  }
  ...
  SOCKET GetSocket() override { return -1; }
...
void PhysicalSocketServer::WakeUp() { signal_wakeup_->Signal(); }
...
// http://crbug.com/344505
  ...
  while (fWait_) {
    int fdmax = -1;
    {
      ...
      assert(!processing_dispatchers_);
...
void PhysicalSocketServer::AddEpoll(Dispatcher* pdispatcher) {
  ...
  assert(err == 0);
  if (err == -1) {
    LOG(ERROR) << "epoll_ctl EPOLL_CTL_ADD";
  }
}
...
void PhysicalSocketServer::RemoveEpoll(Dispatcher* pdispatcher) {
  ...
  if (err == -1) {
    if (errno == ENOENT) {
      ...
      LOG(TRACE) << "epoll_ctl EPOLL_CTL_DEL";
    } else {
      LOG(ERROR) << "epoll_ctl EPOLL_CTL_DEL";
    }
  }
}
...
void PhysicalSocketServer::UpdateEpoll(Dispatcher* pdispatcher) {
  ...
  assert(err == 0);
  if (err == -1) {
    LOG(ERROR) << "epoll_ctl EPOLL_CTL_MOD";
  }
}
  ...
  while (fWait_) {
...

    {
      ...
      for (Dispatcher* disp : dispatchers_) {
        if (!process_io && (disp != signal_wakeup_)) continue;
      ...
      if (index > 0) {
        ...
        for (Dispatcher* disp : dispatchers_) {
          ...
          if (err == 0) {
            ...
            uint32_t ff = 0;
            int errcode = 0;
            if (wsaEvents.lNetworkEvents & FD_READ) ff |= DE_READ;
            if (wsaEvents.lNetworkEvents & FD_WRITE) ff |= DE_WRITE;
            if (wsaEvents.lNetworkEvents & FD_CONNECT) {
