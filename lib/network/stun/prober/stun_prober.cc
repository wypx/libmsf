...
class StunProber::Requester : public sigslot::has_slots<> {
 public:
  ...
  Requester(StunProber* prober,
            ...
            const std::vector<SocketAddress>& server_ips);
  ~Requester() override;
  ...
  void OnStunResponseReceived(AsyncPacketSocket* socket,
                              const char* buf,
                              size_t size,
 ...
 private:
  Request* GetRequestByAddress(const IPAddress& ip);
  ...
  SocketAddress addr_;
  std::unique_ptr<ByteBufferWriter> response_packet_;

  std::vector<Request*> requests_;
  std::vector<SocketAddress> server_ips_;
...
StunProber::Requester::Requester(
    ...
    : prober_(prober),
      socket_(socket),
      response_packet_(new ByteBufferWriter(nullptr, kMaxUdpBufferSize)),
...
void StunProber::Requester::SendStunRequest() {
  ...
  message.SetTransactionID(
      CreateRandomString(cricket::kStunTransactionIdLength));
  message.SetType(cricket::STUN_BINDING_REQUEST);

  std::unique_ptr<ByteBufferWriter> request_packet(
      new ByteBufferWriter(nullptr, kMaxUdpBufferSize));
  ...
  request.sent_time_ms = TimeMillis();
...
void StunProber::Requester::Request::ProcessResponse(const char* buf,
  ...
  ByteBufferReader message(buf, buf_len);
...
StunProber::StunProber(PacketSocketFactory* socket_factory,
                       Thread* thread,
                       const NetworkManager::NetworkList& networks)
    : interval_ms_(0),
...
bool StunProber::Start(const std::vector<SocketAddress>& servers,
...
bool StunProber::Prepare(const std::vector<SocketAddress>& servers,
                         bool shared_socket_mode,
                         int interval_ms,
                         int num_request_per_ip,
                         int timeout_ms,
...
bool StunProber::ResolveServerName(const SocketAddress& addr) {
...
void StunProber::OnSocketReady(AsyncPacketSocket* socket,
                               const SocketAddress& addr) {
...
void StunProber::OnServerResolved(AsyncResolverInterface* resolver) {
  RTC_DCHECK(thread_checker_.IsCurrent());

  if (resolver->GetError() == 0) {
    SocketAddress addr(resolver->address().ipaddr(),
  ...
  invoker_.AsyncInvoke<void>(
      ...
      Bind(&AsyncResolverInterface::Destroy, resolver, false));
  servers_.pop_back();
...
void StunProber::CreateSockets() {
  // Dedupe.
  std::set<SocketAddress> addrs(all_servers_addrs_.begin(),
  ...
  while (sockets_.size() < total_socket_required()) {
    std::unique_ptr<AsyncPacketSocket> socket(
        socket_factory_->CreateUdpSocket(SocketAddress(INADDR_ANY, 0), 0,
                                         0));
    if (!socket) {
    ...
    if (socket->GetState() == AsyncPacketSocket::STATE_BINDING) {
      socket->SignalAddressReady.connect(this, &StunProber::OnSocketReady);
    } else {
      OnSocketReady(socket.get(), SocketAddress(INADDR_ANY, 0));
    }
...
StunProber::Requester* StunProber::CreateRequester() {
  ...
  if (shared_socket_mode_) {
    ...
    std::vector<SocketAddress> server_ip;
    server_ip.push_back(
...
void StunProber::MaybeScheduleStunRequests() {
  ...
  if (Done()) {
    invoker_.AsyncInvokeDelayed<void>(
        RTC_FROM_HERE, thread_,
        Bind(&StunProber::ReportOnFinished, this, SUCCESS), timeout_ms_);
    return;
  }
  ...
  invoker_.AsyncInvokeDelayed<void>(
      RTC_FROM_HERE, thread_,
      Bind(&StunProber::MaybeScheduleStunRequests, this),
...
bool StunProber::GetStats(StunProber::Stats* prob_stats) const {
  ...
  std::map<IPAddress, int> num_response_per_server;
  std::map<IPAddress, int> num_request_per_server;

  for (auto* requester : requesters_) {
    std::map<SocketAddress, int> num_response_per_srflx_addr;
