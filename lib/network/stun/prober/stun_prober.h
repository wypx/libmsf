...
typedef Callback2<void, StunProber*, int> AsyncCallback;
...
class StunProber : public has_slots<> {
 public:
  ...
  StunProber(PacketSocketFactory* socket_factory,
             Thread* thread,
  ...
  bool Start(const std::vector<SocketAddress>& servers,
  ...
  bool Prepare(const std::vector<SocketAddress>& servers,
               bool shared_socket_mode,
               int stun_ta_interval_ms,
               int requests_per_ip,
               int timeout_ms,
 ...
 private:
  ...
  bool ResolveServerName(const SocketAddress& addr);
  void OnServerResolved(AsyncResolverInterface* resolver);

  void OnSocketReady(AsyncPacketSocket* socket,
  ...
  std::vector<SocketAddress> servers_;
  ...
  std::vector<SocketAddress> all_servers_addrs_;
  ...
  std::vector<AsyncPacketSocket*> sockets_;
  ...
  NetworkManager::NetworkList networks_;
