...
ProxyServer::ProxyServer(SocketFactory* int_factory,
  ...
  assert(server_socket_.get() != nullptr);
...
void ProxyServer::OnAcceptEvent(AsyncSocket* socket) {
  ...
  if (ext_socket) {
    ...
    LOG(ERROR) << "Unable to create external socket on proxy accept event";
  }
}
...
void ProxyBinding::OnInternalClose(AsyncSocket* socket, int err) { Destroy(); }
...
void ProxyBinding::Destroy() { SignalDestroyed(this); }
