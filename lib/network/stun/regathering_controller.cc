...
BasicRegatheringController::BasicRegatheringController(
    const Config& config, IceTransportInternal* ice_transport, EventLoop* loop)
    : config_(config), ice_transport_(ice_transport), loop_(loop) {
  assert(ice_transport_);
  assert(loop_);
...
void
BasicRegatheringController::ScheduleRecurringRegatheringOnFailedNetworks() {
  assert(config_.regather_on_failed_networks_interval >= 0);
  CancelScheduledRecurringRegatheringOnFailedNetworks();
