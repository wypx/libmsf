...
void StreamInterface::PostEvent(EventLoop* loop, int events, int err) {
...
bool StreamInterface::Flush() { return false; }
...
StreamResult StreamAdapterInterface::Read(void* buffer, size_t buffer_len,
                                          size_t* read, int* error) {
  return stream_->Read(buffer, buffer_len, read, error);
}
StreamResult StreamAdapterInterface::Write(const void* data, size_t data_len,
                                           size_t* written, int* error) {
  return stream_->Write(data, data_len, written, error);
}
void StreamAdapterInterface::Close() { stream_->Close(); }
...
void StreamAdapterInterface::Attach(StreamInterface* stream, bool owned) {
  if (nullptr != stream_) stream_->SignalEvent.disconnect(this);
  if (owned_) delete stream_;
  stream_ = stream;
  owned_ = owned;
  if (nullptr != stream_)
    stream_->SignalEvent.connect(this, &StreamAdapterInterface::OnEvent);
...
void StreamAdapterInterface::OnEvent(StreamInterface* stream, int events,
                                     int err) {
