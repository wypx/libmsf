...
class StreamInterface : public MessageHandler {
 public:
  ...
  // re-entrancy.
  void PostEvent(EventLoop* loop, int events, int err);
  ...
  StreamResult WriteAll(const void* data, size_t data_len, size_t* written,
                        int* error);
...
class StreamAdapterInterface : public StreamInterface, public has_slots<> {
 public:
