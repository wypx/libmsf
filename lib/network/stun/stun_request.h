...
class StunRequestManager {
 public:
  explicit StunRequestManager(EventLoop* loop);
  ~StunRequestManager();
  ...
  signal3<const void*, size_t, StunRequest*> SignalSendPacket;
...
class StunRequest : public MessageHandler {
 public:
  StunRequest() = default;
  StunRequest(EventLoop* loop);
  explicit StunRequest(EventLoop* loop, StunMessage* request);
  ~StunRequest();
 ...
 protected:
  ...
  // add luotang
  friend void resend_cb(EventLoop* loop, TimerId timeid, void* data);
