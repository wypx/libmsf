...
TCPPort::TCPPort(EventLoop* loop, PacketSocketFactory* factory,
                 Network* network, uint16_t min_port, uint16_t max_port,
                 const std::string& username, const std::string& password,
                 bool allow_listen)
    : Port(/*loop,*/ LOCAL_PORT_TYPE, factory, network, min_port, max_port,
           username, password),
...
TCPPort::~TCPPort() {
  ...
  for (it = incoming_.begin(); it != incoming_.end(); ++it) delete it->socket;
  incoming_.clear();
}
...
Connection* TCPPort::CreateConnection(const Candidate& address,
  ...
  if (origin == ORIGIN_OTHER_PORT) return NULL;
...
void TCPPort::PrepareAddress() {
  if (socket_) {
    ...
    LOG(TRACE) << "Preparing TCP address, current state: "
               << socket_->GetState();
    if (socket_->GetState() == AsyncPacketSocket::STATE_BOUND ||
        socket_->GetState() == AsyncPacketSocket::STATE_CLOSED)
    ...
    LOG(INFO) << ToString() << ": Not listening due to firewall restrictions.";
...
int TCPPort::SendTo(const void* data, size_t size, const SocketAddress& addr,
  ...
  if (sent < 0) {
    ...
    LOG(ERROR) << ToString() << ": TCP send of " << size
...
int TCPPort::GetError() { return error_; }
...
ProtocolType TCPPort::GetProtocol() const { return PROTO_TCP; }

void TCPPort::OnNewConnection(AsyncPacketSocket* socket,
  ...
  LOG(TRACE) << ToString() << ": Accepted connection from "
...
void TCPPort::OnReadPacket(AsyncPacketSocket* socket, const char* data,
...
void TCPPort::OnAddressReady(AsyncPacketSocket* socket,
                             const SocketAddress& address) {
  AddAddress(address, address, SocketAddress(), TCP_PROTOCOL_NAME, "",
...
TCPConnection::TCPConnection(TCPPort* port, const Candidate& candidate,
  ...
  if (outgoing_) {
    ...
    LOG(TRACE) << ToString() << ": socket ipaddr: "
               << socket_->GetLocalAddress().ToSensitiveString()
               << ", port() Network:" << port->network()->ToString();
    auto ips = port_->network()->GetIPs();
    assert(std::any_of(ips.begin(), ips.end(),
                       [this](const InterfaceAddress& addr) {
...
int TCPConnection::GetError() { return error_; }
...
void TCPConnection::OnConnect(AsyncPacketSocket* socket) {
  ...
  auto ips = port_->network()->GetIPs();
  if (std::any_of(ips.begin(), ips.end(),
                  [socket_address](const InterfaceAddress& addr) {
        return socket_address.ipaddr() == addr;
      })) {
    LOG(TRACE) << ToString() << ": Connection established to "
               << socket->GetRemoteAddress().ToSensitiveString();
  } else {
    if (socket->GetLocalAddress().IsLoopbackIP()) {
      LOG(WARNING) << "Socket is bound to the address:"
      ...
      LOG(WARNING) << "Dropping connection as TCP socket bound to IP "
                   << socket_address.ipaddr().ToSensitiveString()
                   << ", rather than an address associated with network:"
                   << port_->network()->ToString();
      OnClose(socket, 0);
...
void TCPConnection::OnClose(AsyncPacketSocket* socket, int error) {
  assert(socket == socket_.get());
  LOG(INFO) << ToString() << ": Connection closed with error " << error;
...
void TCPConnection::OnMessage(Message* pmsg) {
...
void TCPConnection::MaybeReconnect() {
  ...
  LOG(INFO) << ToString() << ": TCP Connection with remote is closed, "
                             "trying to reconnect";
...
void TCPConnection::OnReadPacket(AsyncPacketSocket* socket, const char* data,
                                 size_t size, const SocketAddress& remote_addr,
                                 const int64_t& packet_time_us) {
  assert(socket == socket_.get());
...
void TCPConnection::CreateOutgoingTcpSocket() {
  ...
  socket_.reset(port()->socket_factory()->CreateClientTcpSocket(
      ...
      tcp_opts));
  if (socket_) {
    LOG(TRACE) << ToString() << ": Connecting from "
               << socket_->GetLocalAddress().ToSensitiveString() << " to "
               << remote_candidate().address().ToSensitiveString();
    set_connected(false);
    connection_pending_ = true;
    ConnectSocketSignals(socket_.get());
  } else {
    LOG(WARNING) << ToString() << ": Failed to create connection to "
                 << remote_candidate().address().ToSensitiveString();
