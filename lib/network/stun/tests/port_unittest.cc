...
SocketAddress GetAddress(Port* port) { return GetCandidate(port).address(); }
...
class TestPort : public Port {
 public:
  TestPort(rtc::Thread* thread, const std::string& type,
      ...
      : Port(thread, type, factory, network, min_port, max_port,
  ...
  void AddCandidateAddress(const rtc::SocketAddress& addr,
                           ...
                           const std::string& type, int type_preference,
                           bool final) {
...
static void SendPingAndReceiveResponse(Connection* lconn, TestPort* lport,
                                       Connection* rconn, TestPort* rport,
...
class TestChannel : public sigslot::has_slots<> {
 public:
  ...
  void OnUnknownAddress(PortInterface* port, const SocketAddress& addr,
                        ProtocolType proto, IceMessage* msg,
...
class PortTest : public ::testing::Test, public sigslot::has_slots<> {
 ...
 protected:
  ...
  void TestConnectivity(const char* name1, std::unique_ptr<Port> port1,
                        const char* name2, std::unique_ptr<Port> port2,
                        bool accept, bool same_addr1, bool same_addr2,
                        bool possible);
  ...
  std::unique_ptr<IceMessage> CreateStunMessageWithUsername(
      int type, const std::string& username) {
...
void PortTest::TestConnectivity(const char* name1, std::unique_ptr<Port> port1,
                                const char* name2, std::unique_ptr<Port> port2,
                                bool accept, bool same_addr1, bool same_addr2,
                                bool possible) {
  rtc::ScopedFakeClock clock;
  ...
  if (accept) {
    ...
    if (same_addr1) EXPECT_EQ(ch2.remote_address(), GetAddress(ch1.port()));
...
// Local -> XXXX
TEST_F(PortTest, TestLocalToLocal) { TestLocalToLocal(); }

TEST_F(PortTest, TestLocalToConeNat) { TestLocalToStun(NAT_OPEN_CONE); }

TEST_F(PortTest, TestLocalToARNat) { TestLocalToStun(NAT_ADDR_RESTRICTED); }

TEST_F(PortTest, TestLocalToPRNat) { TestLocalToStun(NAT_PORT_RESTRICTED); }

TEST_F(PortTest, TestLocalToSymNat) { TestLocalToStun(NAT_SYMMETRIC); }

// Flaky: https://code.google.com/p/webrtc/issues/detail?id=3316.
TEST_F(PortTest, DISABLED_TestLocalToTurn) { TestLocalToRelay(PROTO_UDP); }

// Cone NAT -> XXXX
TEST_F(PortTest, TestConeNatToLocal) { TestStunToLocal(NAT_OPEN_CONE); }
...
TEST_F(PortTest, TestARNatToLocal) { TestStunToLocal(NAT_ADDR_RESTRICTED); }
...
TEST_F(PortTest, TestPRNatToLocal) { TestStunToLocal(NAT_PORT_RESTRICTED); }
...
// Symmetric NAT -> XXXX
TEST_F(PortTest, TestSymNatToLocal) { TestStunToLocal(NAT_SYMMETRIC); }
...
// Outbound TCP -> XXXX
TEST_F(PortTest, TestTcpToTcp) { TestTcpToTcp(); }
...
TEST_F(PortTest, TestSkipCrossFamilyTcp) { TestCrossFamilyPorts(SOCK_STREAM); }

TEST_F(PortTest, TestSkipCrossFamilyUdp) { TestCrossFamilyPorts(SOCK_DGRAM); }
...
INSTANTIATE_TEST_SUITE_P(GoogPingTest, GoogPingTest,
...
class ConnectionTest : public PortTest {
 public:
  ...
  void SendPingAndCaptureReply(Connection* lconn, Connection* rconn, int64_t ms,
  ...
  void SendPingAndReceiveResponse(Connection* lconn, Connection* rconn,
                                  int64_t ms) {
    rtc::BufferT<uint8_t> reply;
