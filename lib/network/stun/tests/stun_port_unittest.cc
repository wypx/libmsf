...
class MockAsyncPacketSocket : public rtc::AsyncPacketSocket {
 public:
  ...
  MOCK_METHOD(int, GetOption, (rtc::Socket::Option opt, int* value),
              (override));
...
TEST_F(StunPortTest, TestStunPacketsHaveDscpPacketOption) {
  ...
  EXPECT_CALL(*socket, SendTo(_, _, _, ::testing::Field(
                                           &rtc::PacketOptions::dscp,
  ...
  EXPECT_CALL(*socket,
              SendTo(_, _, _, ::testing::Field(&rtc::PacketOptions::dscp,
