...
class TestUDPPort : public UDPPort {
 public:
  static TestUDPPort* Create(rtc::Thread* thread,
                             ...
                             rtc::Network* network, uint16_t min_port,
 ...
 protected:
  TestUDPPort(rtc::Thread* thread, rtc::PacketSocketFactory* factory,
              rtc::Network* network, uint16_t min_port, uint16_t max_port,
              const std::string& username, const std::string& password,
              const std::string& origin, bool emit_localhost_for_anyaddress)
      : UDPPort(thread, factory, network, min_port, max_port, username,
                password, origin, emit_localhost_for_anyaddress) {}
};
 ...
 public:
  FakePortAllocatorSession(PortAllocator* allocator,
      ...
      : PortAllocatorSession(content_name, component, ice_ufrag, ice_pwd,
        ...
        factory_(factory),
        ipv4_network_("network", "unittest", rtc::IPAddress(INADDR_LOOPBACK),
                      32),
        ipv6_network_("network", "unittest", rtc::IPAddress(in6addr_loopback),
                      64),
        port_(),
