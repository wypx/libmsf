...
bool LocalCandidateUsesPreferredNetwork(
    const Connection* conn, std::optional<AdapterType> network_preference) {
...
const Connection* BasicIceController::FindNextPingableConnection() {
  ...
  if (weak()) {
    ...
    auto conns = GetBestWritableConnectionPerNetwork();
    std::copy_if(conns.begin(), conns.end(),
    ...
    auto iter =
        std::min_element(pingable_selectable_connections.begin(),
  ...
  if (std::none_of(unpinged_connections_.begin(), unpinged_connections_.end(),
                   [this, now](const Connection* conn) {
  ...
  std::copy_if(
      unpinged_connections_.begin(), unpinged_connections_.end(),
      std::back_inserter(pingable_connections),
      [this, now](const Connection* conn) { return IsPingable(conn, now); });
  auto iter = std::max_element(
      pingable_connections.begin(), pingable_connections.end(),
...
const Connection* BasicIceController::FindOldestConnectionNeedingTriggeredCheck(
  ...
  if (oldest_needing_triggered_check) {
    LOG(INFO) << "Selecting connection for triggered check: "
...
bool BasicIceController::WritableConnectionPastPingInterval(
    const Connection* conn, int64_t now) const {
...
BasicIceController::HandleInitialSelectDampening(
    IceControllerEvent reason, const Connection* new_connection) {
  ...
  if (now >= max_wait_until) {
    LOG(INFO) << "reset initial_select_timestamp_ = "
  ...
  if (initial_select_timestamp_ms_ == 0) {
    ...
    LOG(INFO) << "set initial_select_timestamp_ms_ = "
              << initial_select_timestamp_ms_;
  }
  ...
  LOG(INFO) << "delay initial selection up to " << min_delay << "ms";
...
IceControllerInterface::SwitchResult BasicIceController::ShouldSwitchConnection(
  ...
  std::optional<IceControllerEvent> recheck_event;
...
BasicIceController::SortAndSwitchConnection(IceControllerEvent reason) {
  ...
  LOG(TRACE) << "Sorting " << connections_.size() << " available connections";
  for (size_t i = 0; i < connections_.size(); ++i) {
    LOG(TRACE) << connections_[i]->ToString();
  }
