...
struct NetworkFilter {
  using Predicate = std::function<bool(Network*)>;
  NetworkFilter(Predicate pred, const std::string& description)
      : predRemain([pred](Network* network) { return !pred(network); }),
        description(description) {}
  Predicate predRemain;
  const std::string description;
};

using NetworkList = NetworkManager::NetworkList;
void FilterNetworks(NetworkList* networks, NetworkFilter filter) {
  ...
  LOG(INFO) << "Filtered out " << filter.description << " networks:";
...
BasicPortAllocator::BasicPortAllocator(
    NetworkManager* network_manager, PacketSocketFactory* socket_factory,
    TurnCustomizer* customizer, RelayPortFactoryInterface* relay_port_factory)
    : network_manager_(network_manager), socket_factory_(socket_factory) {
  InitRelayPortFactory(relay_port_factory);
  assert(relay_port_factory_ != nullptr);
...
BasicPortAllocator::BasicPortAllocator(NetworkManager* network_manager,
    ...
    : BasicPortAllocator(network_manager,
                         /*socket_factory=*/nullptr, stun_servers) {}
...
BasicPortAllocator::BasicPortAllocator(NetworkManager* network_manager,
  ...
  InitRelayPortFactory(nullptr);
  assert(relay_port_factory_ != nullptr);
  SetConfiguration(stun_servers, std::vector<RelayServerConfig>(), 0, NO_PRUNE,
                   nullptr);
}
...
BasicPortAllocatorSession::BasicPortAllocatorSession(
    BasicPortAllocator* allocator, const std::string& content_name,
...
BasicPortAllocatorSession::~BasicPortAllocatorSession() {
  ...
  // if (network_thread_ != NULL) network_thread_->Clear(this);
  ...
  for (it = ports_.begin(); it != ports_.end(); it++) delete it->port();

  for (uint32_t i = 0; i < configs_.size(); ++i) delete configs_[i];

  for (uint32_t i = 0; i < sequences_.size(); ++i) delete sequences_[i];
}
...
void BasicPortAllocatorSession::StartGettingPorts() {
  ...
  if (!socket_factory_) {
    owned_socket_factory_.reset(new BasicPacketSocketFactory(loop_));
    socket_factory_ = owned_socket_factory_.get();
  }

  // network_thread_->Post(RTC_FROM_HERE, this, MSG_CONFIG_START);

  LOG(INFO) << "Start getting ports with turn_port_prune_policy "
...
std::vector<Network*> BasicPortAllocatorSession::GetFailedNetworks() {
  // RTC_DCHECK_RUN_ON(network_thread_);
  ...
  networks.erase(std::remove_if(networks.begin(), networks.end(),
                                [networks_with_connection](Network* network) {
...
void BasicPortAllocatorSession::RegatherOnFailedNetworks() {
  ...
  std::vector<Network*> failed_networks = GetFailedNetworks();
  if (failed_networks.empty()) {
    return;
  }

  LOG(INFO) << "Regather candidates on failed networks";
...
void BasicPortAllocatorSession::Regather(const std::vector<Network*>& networks,
  ...
  if (!ports_to_prune.empty()) {
    LOG(INFO) << "Prune " << ports_to_prune.size() << " ports";
...
void BasicPortAllocatorSession::SetStunKeepaliveIntervalForReadyPorts(
  ...
  for (PortInterface* port : ports) {
    ...
    if (port->Type() == STUN_PORT_TYPE ||
      ...
      static_cast<UDPPort*>(port)
          ->set_stun_keepalive_delay(stun_keepalive_interval);
    }
  }
}
...
void BasicPortAllocatorSession::GetCandidatesFromPort(
    const PortData& data, std::vector<Candidate>* candidates) const {
  // RTC_DCHECK_RUN_ON(network_thread_);
...
std::vector<Network*> BasicPortAllocatorSession::GetNetworks() {
  // RTC_DCHECK_RUN_ON(network_thread_);
  ...
  if (flags() & PORTALLOCATOR_DISABLE_LINK_LOCAL_NETWORKS) {
    NetworkFilter link_local_filter([](Network* network) {
  ...
  NetworkFilter ignored_filter([this](Network* network) {
  ...
  if (flags() & PORTALLOCATOR_DISABLE_COSTLY_NETWORKS) {
    ...
    NetworkFilter costly_filter([lowest_cost](Network* network) {
...
void BasicPortAllocatorSession::DoAllocate(bool disable_equivalent) {
  ...
  std::vector<Network*> networks = GetNetworks();
  if (networks.empty()) {
    LOG(WARNING) << "Machine has no networks; no ports will be allocated";
    done_signal_needed = true;
  } else {
    LOG(INFO) << "Allocate ports on " << networks.size() << " networks";
...
void BasicPortAllocatorSession::OnNetworksChanged() {
  // RTC_DCHECK_RUN_ON(network_thread_);
  std::vector<Network*> networks = GetNetworks();
  std::vector<Network*> failed_networks;
  for (AllocationSequence* sequence : sequences_) {
  ...
  if (!ports_to_prune.empty()) {
    LOG(INFO) << "Prune " << ports_to_prune.size()
  ...
  if (!network_manager_started_) {
    LOG(INFO) << "Network manager has started";
    network_manager_started_ = true;
  }
}

void BasicPortAllocatorSession::DisableEquivalentPhases(
    Network* network, PortConfiguration* config, uint32_t* flags) {
  // RTC_DCHECK_RUN_ON(network_thread_);
...
void BasicPortAllocatorSession::AddAllocatedPort(Port* port,
  ...
  LOG(INFO) << "Adding allocated port for " << content_name();
  ...
  LOG(INFO) << port->ToString() << ": Added port to allocator";

  if (prepare_address) port->PrepareAddress();
}
...
void BasicPortAllocatorSession::OnCandidateReady(Port* port,
  ...
  if (CandidatePairable(c, port) && !data->has_pairable_candidate()) {
    ...
    if (!data->pruned()) {
      LOG(INFO) << port->ToString() << ": Port ready.";
  ...
  if (data->ready() && CheckCandidateFilter(c)) {
    ...
    LOG(INFO) << "Discarding candidate because it doesn't match filter.";
  }
...
void BasicPortAllocatorSession::OnCandidateError(
    Port* port, const IceCandidateErrorEvent& event) {
  // RTC_DCHECK_RUN_ON(network_thread_);
  assert(FindPort(port));
...
bool BasicPortAllocatorSession::PruneNewlyPairableTurnPort(
  ...
  // RTC_DCHECK_RUN_ON(network_thread_);
  assert(newly_pairable_port_data->port()->Type() == RELAY_PORT_TYPE);
...
void BasicPortAllocatorSession::OnPortComplete(Port* port) {
  // RTC_DCHECK_RUN_ON(network_thread_);
  LOG(INFO) << port->ToString() << ": Port completed gathering candidates.";
  PortData* data = FindPort(port);
  assert(data != NULL);
...
void BasicPortAllocatorSession::MaybeSignalCandidatesAllocationDone() {
  ...
  if (CandidatesAllocationDone()) {
    if (pooled()) {
      LOG(INFO) << "All candidates gathered for pooled session.";
    } else {
      LOG(INFO) << "All candidates gathered for " << content_name() << ":"
...
void BasicPortAllocatorSession::OnPortDestroyed(PortInterface* port) {
  ...
  for (std::vector<PortData>::iterator iter = ports_.begin();
    ...
    if (port == iter->port()) {
      ports_.erase(iter);
      LOG(INFO) << port->ToString() << ": Removed port from allocator ("
...
void BasicPortAllocatorSession::PrunePortsAndRemoveCandidates(
  ...
  if (!removed_candidates.empty()) {
    LOG(INFO) << "Removed " << removed_candidates.size() << " candidates";
...
void AllocationSequence::DisableEquivalentPhases(Network* network,
                                                 PortConfiguration* config,
                                                 uint32_t* flags) {
  if (network_failed_) {
...
void AllocationSequence::CreateUDPPorts() {
  if (IsFlagSet(PORTALLOCATOR_DISABLE_UDP)) {
    LOG(TRACE) << "AllocationSequence: UDP ports disabled, skipping.";
    return;
  }
  ...
  if (IsFlagSet(PORTALLOCATOR_ENABLE_SHARED_SOCKET) && udp_socket_) {
    port = UDPPort::Create(
        /*loop_,*/ session_->socket_factory(), network_, udp_socket_.get(),
        session_->username(), session_->password(),
        session_->allocator()->origin(), emit_local_candidate_for_anyaddress,
        session_->allocator()->stun_candidate_keepalive_interval());
  } else {
    port = UDPPort::Create(
        /*loop_,*/ session_->socket_factory(), network_,
        session_->allocator()->min_port(), session_->allocator()->max_port(),
        session_->username(), session_->password(),
        session_->allocator()->origin(), emit_local_candidate_for_anyaddress,
        session_->allocator()->stun_candidate_keepalive_interval());
    ...
    if (IsFlagSet(PORTALLOCATOR_ENABLE_SHARED_SOCKET)) {
      ...
      if (!IsFlagSet(PORTALLOCATOR_DISABLE_STUN)) {
        if (config_ && !config_->StunServers().empty()) {
          LOG(INFO) << "AllocationSequence: UDPPort will be handling the "
...
void AllocationSequence::CreateTCPPorts() {
  if (IsFlagSet(PORTALLOCATOR_DISABLE_TCP)) {
    LOG(TRACE) << "AllocationSequence: TCP ports disabled, skipping.";
    return;
  }
...
void AllocationSequence::CreateStunPorts() {
  if (IsFlagSet(PORTALLOCATOR_DISABLE_STUN)) {
    LOG(TRACE) << "AllocationSequence: STUN ports disabled, skipping.";
    return;
  }
  ...
  if (!(config_ && !config_->StunServers().empty())) {
    LOG(WARNING) << "AllocationSequence: No STUN server configured, skipping.";
    return;
  }
...
void AllocationSequence::CreateRelayPorts() {
  if (IsFlagSet(PORTALLOCATOR_DISABLE_RELAY)) {
    LOG(TRACE) << "AllocationSequence: Relay ports disabled, skipping.";
    return;
  }
  ...
  assert(config_);
  assert(!config_->relays.empty());
  if (!(config_ && !config_->relays.empty())) {
    LOG(WARNING) << "AllocationSequence: No relay server configured, skipping.";
    return;
  }
...
void AllocationSequence::CreateTurnPort(const RelayServerConfig& config) {
  ...
  for (relay_port = config.ports.begin(); relay_port != config.ports.end();
    ...
    if (server_ip_family != AF_UNSPEC && server_ip_family != local_ip_family) {
      LOG(INFO) << "Server and local address families are not compatible. "
                   "Server address: "
                << relay_port->address.ipaddr().ToSensitiveString()
                << " Local address: "
                << network_->GetBestIP().ToSensitiveString();
      continue;
    }
    ...
    if (IsFlagSet(PORTALLOCATOR_ENABLE_SHARED_SOCKET) &&
      ...
      if (!port) {
        LOG(WARNING) << "Failed to create relay port with "
...
void AllocationSequence::OnPortDestroyed(PortInterface* port) {
  ...
  if (it != relay_ports_.end()) {
    ...
    LOG(ERROR) << "Unexpected OnPortDestroyed for nonexistent port.";
    // RTC_NOTREACHED();
  }
}

// PortConfiguration
PortConfiguration::PortConfiguration(const SocketAddress& stun_address,
                                     const std::string& username,
                                     const std::string& password)
    : stun_address(stun_address), username(username), password(password) {
  if (!stun_address.IsNil()) stun_servers.insert(stun_address);
}
...
PortConfiguration::PortConfiguration(const ServerAddresses& stun_servers,
  ...
  if (!stun_servers.empty()) stun_address = *(stun_servers.begin());
  // Note that this won't change once the config is initialized.
  // use_turn_server_as_stun_server_disabled =
  //    field_trial::IsDisabled("WebRTC-UseTurnServerAsStunServer");
}
...
bool PortConfiguration::SupportsProtocol(const RelayServerConfig& relay,
  ...
  for (relay_port = relay.ports.begin(); relay_port != relay.ports.end();
       ++relay_port) {
    if (relay_port->proto == type) return true;
  }
  return false;
}
...
bool PortConfiguration::SupportsProtocol(ProtocolType type) const {
  for (size_t i = 0; i < relays.size(); ++i) {
    if (SupportsProtocol(relays[i], type)) return true;
  }
  return false;
}

ServerAddresses PortConfiguration::GetRelayServerAddresses(ProtocolType type)
    const {
