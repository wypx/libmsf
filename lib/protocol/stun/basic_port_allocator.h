...
#include "relay_port_factory_interface.h"
...
class BasicPortAllocator : public PortAllocator {
 public:
  ...
  BasicPortAllocator(NetworkManager* network_manager,
                     ...
                     TurnCustomizer* customizer = nullptr,
                     RelayPortFactoryInterface* relay_port_factory = nullptr);
  explicit BasicPortAllocator(NetworkManager* network_manager);
  BasicPortAllocator(NetworkManager* network_manager,
  ...
  PortAllocatorSession* CreateSessionInternal(
      ...
      const std::string& ice_ufrag, const std::string& ice_pwd) override;
...
class BasicPortAllocatorSession : public PortAllocatorSession,
 ...
 public:
  ...
  void GetCandidateStatsFromReadyPorts(CandidateStatsList* candidate_stats_list)
      const override;
 ...
 private:
  class PortData {
   public:
    ...
    void set_state(State state) {
      assert(state != STATE_ERROR || state_ == STATE_INPROGRESS);
      state_ = state;
    }
  ...
  void DisableEquivalentPhases(Network* network, PortConfiguration* config,
                               uint32_t* flags);
  ...
  std::vector<Network*> GetFailedNetworks();
  void Regather(const std::vector<Network*>& networks,
                bool disable_equivalent_phases, IceRegatheringReason reason);
  ...
  std::unique_ptr<PacketSocketFactory> owned_socket_factory_;
  PacketSocketFactory* socket_factory_;
...
class AllocationSequence : public MessageHandler, public has_slots<> {
 public:
  enum State {
    kInit,       // Initial state.
  ...
  AllocationSequence(BasicPortAllocatorSession* session, Network* network,
  ...
  signal1<AllocationSequence*> SignalPortAllocationComplete;
