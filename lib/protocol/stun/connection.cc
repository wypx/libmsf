...
inline bool TooManyFailures(
    ...
    uint32_t maximum_failures, int rtt_estimate, int64_t now) {
  // If we haven't sent that many pings, then we can't have failed that many.
  if (pings_since_last_response.size() < maximum_failures) return false;
...
inline bool TooLongWithoutResponse(
    const std::vector<Connection::SentPing>& pings_since_last_response,
    int64_t maximum_time, int64_t now) {
  if (pings_since_last_response.size() == 0) return false;
...
// enum.
webIceCandidateType GetCandidateTypeByString(const std::string& type) {
  if (type == LOCAL_PORT_TYPE) {
    return webIceCandidateType::kLocal;
  } else if (type == STUN_PORT_TYPE) {
    return webIceCandidateType::kStun;
  } else if (type == PRFLX_PORT_TYPE) {
    return webIceCandidateType::kPrflx;
  } else if (type == RELAY_PORT_TYPE) {
    return webIceCandidateType::kRelay;
  }
  return webIceCandidateType::kUnknown;
}

webIceCandidatePairProtocol GetProtocolByString(const std::string& protocol) {
  if (protocol == UDP_PROTOCOL_NAME) {
    return webIceCandidatePairProtocol::kUdp;
  } else if (protocol == TCP_PROTOCOL_NAME) {
    return webIceCandidatePairProtocol::kTcp;
  } else if (protocol == SSLTCP_PROTOCOL_NAME) {
    return webIceCandidatePairProtocol::kSsltcp;
  } else if (protocol == TLS_PROTOCOL_NAME) {
    return webIceCandidatePairProtocol::kTls;
  }
  return webIceCandidatePairProtocol::kUnknown;
}

webIceCandidatePairAddressFamily GetAddressFamilyByInt(int address_family) {
  if (address_family == AF_INET) {
    return webIceCandidatePairAddressFamily::kIpv4;
  } else if (address_family == AF_INET6) {
    return webIceCandidatePairAddressFamily::kIpv6;
  }
  return webIceCandidatePairAddressFamily::kUnknown;
}

webIceCandidateNetworkType ConvertNetworkType(AdapterType type) {
  switch (type) {
    case ADAPTER_TYPE_ETHERNET:
      return webIceCandidateNetworkType::kEthernet;
    case ADAPTER_TYPE_LOOPBACK:
      return webIceCandidateNetworkType::kLoopback;
    case ADAPTER_TYPE_WIFI:
      return webIceCandidateNetworkType::kWifi;
    case ADAPTER_TYPE_VPN:
      return webIceCandidateNetworkType::kVpn;
    ...
    case ADAPTER_TYPE_CELLULAR_5G:
      return webIceCandidateNetworkType::kCellular;
    default:
      return webIceCandidateNetworkType::kUnknown;
  }
}
#endif
...
inline int ConservativeRTTEstimate(int rtt) {
  return (2 * rtt <= MINIMUM_RTT)
             ? static_cast<int>(MINIMUM_RTT)
             : ((2 * rtt >= MAXIMUM_RTT) ? static_cast<int>(MAXIMUM_RTT)
                                         : static_cast<int>(2 * rtt));
}
...
ConnectionRequest::ConnectionRequest(Connection* connection)
    : StunRequest(nullptr, new IceMessage()), connection_(connection) {}
...
void ConnectionRequest::Prepare(StunMessage* request) {
  ...
  request->AddAttribute(
      std::make_unique<StunByteStringAttribute>(kStunAttrUsername, username));
  ...
  request->AddAttribute(std::make_unique<StunUInt32Attribute>(
      kStunAttrICENetworkInfo, network_info));

  // if (webfield_trial::IsEnabled("WebRTC-PiggybackIceCheckAcknowledgement") &&
  //     connection_->last_ping_id_received()) {
  //   request->AddAttribute(std::make_unique<StunByteStringAttribute>(
  //       kStunAttrLastIceCheckReceived,
  ...
  if (connection_->port()->GetIceRole() == ICEROLE_CONTROLLING) {
    request->AddAttribute(std::make_unique<StunUInt64Attribute>(
        kStunAttrICEControlling, connection_->port()->IceTiebreaker()));
    ...
    if (connection_->use_candidate_attr()) {
      request->AddAttribute(
          std::make_unique<StunByteStringAttribute>(kStunAttrICEUseCandidate));
    }
    if (connection_->nomination() &&
        connection_->nomination() != connection_->acked_nomination()) {
      request->AddAttribute(std::make_unique<StunUInt32Attribute>(
          kStunAttrICENomination, connection_->nomination()));
    }
  } else if (connection_->port()->GetIceRole() == ICEROLE_CONTROLLED) {
    request->AddAttribute(std::make_unique<StunUInt64Attribute>(
        kStunAttrICEControlled, connection_->port()->IceTiebreaker()));
  } else {
    // RTC_NOTREACHED();
  }
  ...
  request->AddAttribute(std::make_unique<StunUInt32Attribute>(
      kStunAttrICEPriority, prflx_priority));
  ...
  if (connection_->field_trials_->enable_goog_ping &&
    ...
    // support. This is sent on all kStunMessageBindingRequest until we get a
    // kStunMessageBindingResponse.
    auto list = StunAttribute::CreateUInt16ListAttribute(kStunAttrGoodMiscInfo);
  ...
  if (connection_->ShouldSendGoogPing(request)) {
    request->SetType(kStunGoodPingRequest);
...
int ConnectionRequest::resend_delay() { return CONNECTION_RESPONSE_TIMEOUT; }
...
Connection::Connection(Port* port, size_t index,
  ...
  LOG(INFO) << ToString() << ": Connection created";
}
...
const Network* Connection::network() const { return port()->network(); }

int Connection::generation() const { return port()->generation(); }
...
void Connection::set_write_state(WriteState value) {
  ...
  write_state_ = value;
  if (value != old_value) {
    LOG(INFO) << ToString() << ": set_write_state from: " << old_value << " to "
              << value;
...
void Connection::UpdateReceiving(int64_t now) {
  ...
  LOG(INFO) << ToString() << ": set_receiving to " << receiving;
...
void Connection::set_connected(bool value) {
  ...
  connected_ = value;
  if (value != old_value) {
    LOG(INFO) << ToString() << ": Change connected_ to " << value;
    SignalStateChange(this);
  }
}
...
void Connection::OnSendStunPacket(const void* data, size_t size,
                                  StunRequest* req) {
  PacketOptions options(port_->StunDscpValue());
  options.info_signaled_after_sent.packet_type =
      PacketType::kIceConnectivityCheck;
  auto err =
      port_->SendTo(data, size, remote_candidate_.address(), options, false);
  if (err < 0) {
    LOG(WARNING) << ToString() << ": Failed to send STUN ping "
                                  " err=" << err
                 << " id=" << hex_encode(req->id());
  }
}
...
void Connection::OnReadPacket(const char* data, size_t size,
  ...
  if (!port_->GetStunMessage(data, size, addr, &msg, &remote_ufrag)) {
    ...
    if (!pruned_ && (write_state_ == STATE_WRITE_TIMEOUT)) {
      LOG(WARNING) << "Received a data packet on a timed-out Connection. "
    ...
    // Log at INFO if we receive a ping on an unwritable connection.

    switch (msg->type()) {
      case kStunMessageBindingRequest:
        LOG(INFO) << ToString() << ": Received "
        ...
        if (remote_ufrag == remote_candidate_.username()) {
          ...
          LOG(ERROR) << ToString()
                     ...
                     << remote_ufrag;
          port_->SendBindingErrorResponse(msg.get(), addr, kStunNotAuthorized,
                                          "Not Authorized");
        }
        break;
      ...
      // id's match.
      case kStunMessageBindingResponse:
      case kStunMessageBindingErrorResponse:
      ...
      case kStunMessageBindingIndication:
        ReceivedPing(msg->transaction_id());
        break;
      case kStunGoodPingRequest:
        HandleStunBindingOrGoogPingRequest(msg.get());
        break;
      case kStunGoodPingResponse:
      case kStunGoodPingErrorResponse:
...
void Connection::HandleStunBindingOrGoogPingRequest(IceMessage* msg) {
  ...
  if (webfield_trial::IsEnabled("WebRTC-ExtraICEPing") &&
    ...
    if (local_candidate().type() == RELAY_PORT_TYPE ||
      ...
      if (last_ping_sent_ + kMinExtraPingDelayMs <= now) {
        LOG(INFO) << ToString() << "WebRTC-ExtraICEPing/Sending extra ping"
                                   " last_ping_sent_: " << last_ping_sent_
                  << " now: " << now << " (diff: " << (now - last_ping_sent_)
                  << ")";
        Ping(now);
      } else {
        LOG(INFO) << ToString() << "WebRTC-ExtraICEPing/Not sending extra ping"
                                   " last_ping_sent_: " << last_ping_sent_
                  << " now: " << now << " (diff: " << (now - last_ping_sent_)
                  << ")";
      }
  ...
  if (msg->type() == kStunMessageBindingRequest) {
    ...
    if (!port_->MaybeIceRoleConflict(remote_addr, msg, remote_ufrag)) {
      ...
      LOG(INFO) << "Received conflicting role from the peer.";
      return;
    }
  }

  stats_.recv_ping_requests++;
  // LogCandidatePairEvent(webIceCandidatePairEventType::kCheckReceived,
  ...
  if (msg->type() == kStunMessageBindingRequest) {
    ...
    assert(msg->type() == kStunGoodPingRequest);
  ...
  if (port_->GetIceRole() == ICEROLE_CONTROLLED) {
    const StunUInt32Attribute* nomination_attr =
        msg->GetUInt32(kStunAttrICENomination);
    uint32_t nomination = 0;
    if (nomination_attr) {
      nomination = nomination_attr->value();
      if (nomination == 0) {
        LOG(ERROR) << "Invalid nomination: " << nomination;
      }
    } else {
      const StunByteStringAttribute* use_candidate_attr =
          msg->GetByteString(kStunAttrICEUseCandidate);
  ...
  const StunUInt32Attribute* network_attr =
      msg->GetUInt32(kStunAttrICENetworkInfo);
  if (network_attr) {
...
void Connection::SendStunBindingResponse(const StunMessage* request) {
  assert(request->type() == kStunMessageBindingRequest);

  // Retrieve the username from the request.
  const StunByteStringAttribute* username_attr =
      request->GetByteString(kStunAttrUsername);
  ...
  const StunUInt32Attribute* retransmit_attr =
      request->GetUInt32(kStunAttrRetransmitCount);
  if (retransmit_attr) {
    ...
    response.AddAttribute(std::make_unique<StunUInt32Attribute>(
        kStunAttrRetransmitCount, retransmit_attr->value()));
  ...
  response.AddAttribute(std::make_unique<StunXorAddressAttribute>(
      kStunAttrXorMappedAddress, remote_candidate_.address()));

  if (field_trials_->announce_goog_ping) {
    // Check if request contains a announce-request.
    auto goog_misc = request->GetUInt16List(kStunAttrGoodMiscInfo);
    if (goog_misc != nullptr &&
      ...
      auto list =
          StunAttribute::CreateUInt16ListAttribute(kStunAttrGoodMiscInfo);
...
void Connection::SendGoogPingResponse(const StunMessage* request) {
  assert(request->type() == kStunGoodPingRequest);

  // Fill in the response message.
  StunMessage response;
  response.SetType(kStunGoodPingResponse);
  response.SetTransactionID(request->transaction_id());
...
void Connection::SendResponseMessage(const StunMessage& response) {
  ...
  if (err < 0) {
    LOG(ERROR) << ToString() << ": Failed to send "
               << StunMethodToString(response.type())
               << ", to=" << addr.ToSensitiveString() << ", err=" << err
               << ", id=" << hex_encode(response.transaction_id());
  } else {
    // Log at INFO if we send a stun ping response on an unwritable
    // connection.

    LOG(INFO) << ToString() << ": Sent " << StunMethodToString(response.type())
              << ", to=" << addr.ToSensitiveString()
              << ", id=" << hex_encode(response.transaction_id());

    stats_.sent_ping_responses++;
    // LogCandidatePairEvent(webIceCandidatePairEventType::kCheckResponseSent,
    //                       response.reduced_transaction_id());
  }
}

void Connection::OnReadyToSend() { SignalReadyToSend(this); }

void Connection::Prune() {
  if (!pruned_ || active()) {
    LOG(INFO) << ToString() << ": Connection pruned";
    pruned_ = true;
    requests_.Clear();
...
void Connection::Destroy() {
  ...
  LOG(INFO) << ToString() << ": Connection destroyed";
  // port_->thread()->Post(RTC_FROM_HERE, this, MSG_DELETE);
  // LogCandidatePairConfig(webIceCandidatePairConfigType::kDestroyed);
}
...
void Connection::UpdateState(int64_t now) {
  ...
  if ((write_state_ == STATE_WRITABLE) &&
    ...
    LOG(INFO) << ToString() << ": Unwritable after " << max_pings
              << " ping failures and "
              << now - pings_since_last_response_[0].sent_time
              << " ms without a response,"
                 " ms since last received ping=" << now - last_ping_received_
              << " ms since last received data=" << now - last_data_received_
              << " rtt=" << rtt;
  ...
  if ((write_state_ == STATE_WRITE_UNRELIABLE ||
      ...
      TooLongWithoutResponse(pings_since_last_response_, inactive_timeout(),
                             now)) {
    LOG(INFO) << ToString() << ": Timed out after "
              << now - pings_since_last_response_[0].sent_time
...
void Connection::HandlePiggybackCheckAcknowledgementIfAny(StunMessage* msg) {
  assert(msg->type() == kStunMessageBindingRequest ||
         msg->type() == kStunGoodPingRequest);
  const StunByteStringAttribute* last_ice_check_received_attr =
      msg->GetByteString(kStunAttrLastIceCheckReceived);
  if (last_ice_check_received_attr) {
    ...
    if (iter != pings_since_last_response_.end()) {
      ...
      LOG(INFO) << ToString() << ": Received piggyback STUN ping response, id="
...
void Connection::ReceivedPingResponse(
    int rtt, const std::string& request_id,
  ...
  if (rtt_samples_ > 0) {
    rtt_ = GetNextMovingAverage(rtt_, rtt, RTT_RATIO);
  } else {
    rtt_ = rtt;
  }
  rtt_samples_++;
...
std::string Connection::ToString() const {
  const std::string_view CONNECT_STATE_ABBREV[2] = {
      "-",  // not connected (false)
      "C",  // connected (true)
  };
  const std::string_view RECEIVE_STATE_ABBREV[2] = {
      "-",  // not receiving (false)
      "R",  // receiving (true)
  };
  const std::string_view WRITE_STATE_ABBREV[4] = {
      "W",  // STATE_WRITABLE
      "w",  // STATE_WRITE_UNRELIABLE
      "-",  // STATE_WRITE_INIT
      "x",  // STATE_WRITE_TIMEOUT
  };
  const std::string_view ICESTATE[4] = {"W",  // STATE_WAITING
                                        "I",  // STATE_INPROGRESS
                                        "S",  // STATE_SUCCEEDED
                                        "F"   // STATE_FAILED
  };
  const std::string_view SELECTED_STATE_ABBREV[2] = {
...
std::string Connection::ToSensitiveString() const { return ToString(); }

#if 0
const webIceCandidatePairDescription& Connection::ToLogDescription() {
  ...
  log_description_ = webIceCandidatePairDescription();
...
// void Connection::LogCandidatePairConfig(webIceCandidatePairConfigType type) {
...
// void Connection::LogCandidatePairEvent(webIceCandidatePairEventType type,
...
void Connection::OnConnectionRequestResponse(ConnectionRequest* request,
                                             StunMessage* response) {
  // Log at INFO if we receive a ping response on an unwritable
  // connection.
  ...
  //     webIceCandidatePairEventType::kCheckResponseReceived,
  //     response->reduced_transaction_id());

  if (request->msg()->type() == kStunMessageBindingRequest) {
    if (!remote_support_goog_ping_.has_value()) {
      auto goog_misc = response->GetUInt16List(kStunAttrGoodMiscInfo);
      if (goog_misc != nullptr &&
...
void Connection::OnConnectionRequestErrorResponse(ConnectionRequest* request,
  ...
  LOG(WARNING) << ToString() << ": Received "
               << StunMethodToString(response->type())
               << " id=" << hex_encode(request->id()) << " code=" << error_code
               << " rtt=" << request->Elapsed();

  cached_stun_binding_.reset();
  if (error_code == kStunAttrUnknownAttributes ||
      error_code == kStunServerError || error_code == kStunNotAuthorized) {
    // Recoverable error, retry
  } else if (error_code == kStunStaleCredentials) {
    // Race failure, retry
  } else if (error_code == kStunRoleConflict) {
    HandleRoleConflictFromPeer();
  } else if (request->msg()->type() == kStunGoodPingRequest) {
    // Race, retry.
  } else {
...
void Connection::OnConnectionRequestTimeout(ConnectionRequest* request) {
  // Log at INFO if we miss a ping on a writable connection
  LOG(INFO) << ToString() << ": Timing-out STUN ping "
            << hex_encode(request->id()) << " after " << request->Elapsed()
            << " ms";
}

void Connection::OnConnectionRequestSent(ConnectionRequest* request) {
  // Log at INFO if we send a ping on an unwritable connection.
  LOG(INFO) << ToString() << ": Sent "
            << StunMethodToString(request->msg()->type())
            << ", id=" << hex_encode(request->id())
            << ", use_candidate=" << use_candidate_attr()
            << ", nomination=" << nomination();
  stats_.sent_ping_requests_total++;
  // LogCandidatePairEvent(webIceCandidatePairEventType::kCheckSent,
...
void Connection::MaybeSetRemoteIceParametersAndGeneration(
    const IceParameters& ice_params, int generation) {
...
void Connection::OnMessage(Message* pmsg) {
  ...
  LOG(INFO) << "Connection deleted with number of pings sent: "
            << num_pings_sent_;
...
void Connection::MaybeUpdateLocalCandidate(ConnectionRequest* request,
  ...
  const StunAddressAttribute* addr =
      response->GetAddress(kStunAttrXorPeerAddress);
  if (!addr) {
    LOG(WARNING) << "Connection::OnConnectionRequestResponse - "
  ...
  for (size_t i = 0; i < port_->Candidates().size(); ++i) {
    if (port_->Candidates()[i].address() == addr->GetAddress()) {
      if (local_candidate_index_ != i) {
        LOG(INFO) << ToString() << ": Updating local candidate type to srflx.";
  ...
  const StunUInt32Attribute* priority_attr =
      request->msg()->GetUInt32(kStunAttrICEPriority);
  if (!priority_attr) {
    LOG(WARNING) << "Connection::OnConnectionRequestResponse - "
                    "No kStunAttrICEPriority found in the "
                    "stun response message";
    return;
  }
  const uint32_t priority = priority_attr->value();
  std::string id = CreateRandomString(8);
  ...
  LOG(INFO) << ToString() << ": Updating local candidate type to prflx.";
...
bool Connection::ShouldSendGoogPing(const StunMessage* message) {
  if (remote_support_goog_ping_ == true && cached_stun_binding_ &&
      cached_stun_binding_->EqualAttributes(message, [](int type) {
        ...
        // kStunAttrGoodMiscInfo
        return type != kStunAttrFingerprint &&
               type != kStunAttrMessageIntegrity &&
               type != kStunAttrRetransmitCount &&
               type != kStunAttrGoodMiscInfo;
      })) {
    return true;
  }
  return false;
...
void Connection::ForgetLearnedState() {
  LOG(INFO) << ToString() << ": Connection forget learned state";
  requests_.Clear();
  receiving_ = false;
  write_state_ = STATE_WRITE_INIT;
  rtt_estimate_.Reset();
...
int ProxyConnection::GetError() { return error_; }
