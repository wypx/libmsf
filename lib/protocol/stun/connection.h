...
class Connection : public CandidatePairInterface,
 ...
 public:
  ...
  signal1<Connection*> SignalStateChange;

  // Sent when the connection has decided that it is no longer of value.  It
  // will delete itself immediately after this call.
  signal1<Connection*> SignalDestroyed;
  ...
  signal4<Connection*, const char*, size_t, int64_t> SignalReadPacket;

  signal1<Connection*> SignalReadyToSend;
  ...
  void ReceivedPing(const std::optional<std::string>& request_id =
                        std::nullopt);
  ...
  // controlling side.
  signal1<Connection*> SignalNominated;
 ...
 private:
  ...
  std::optional<int> unwritable_timeout_;
  std::optional<int> unwritable_min_checks_;
  std::optional<int> inactive_timeout_;

  bool reported_;
  IceCandidatePairState state_;
  // Time duration to switch from receiving to not receiving.
  std::optional<int> receiving_timeout_;
  int64_t time_created_ms_;
  int num_pings_sent_ = 0;

  // std::optional<IceCandidatePairDescription> log_description_;
  // IceEventLog* ice_event_log_ = nullptr;
  ...
  std::optional<bool> remote_support_goog_ping_;
