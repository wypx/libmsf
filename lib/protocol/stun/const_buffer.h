 ...
 *
 **************************************************************************/
#ifndef BASE_CONST_BUFFER_H_
#define BASE_CONST_BUFFER_H_
...
class BufferT {
 ...
 public:
  ...
  // An empty BufferT.
  BufferT() : size_(0), capacity_(0), data_(nullptr) { assert(IsConsistent()); }
  ...
  template <typename U, typename std::enable_if<internal::BufferCompat<
  ...
  template <typename U = T, typename std::enable_if<internal::BufferCompat<
                                T, U>::value>::type* = nullptr>
  const U* data() const {
    assert(IsConsistent());
  ...
  template <typename W, typename std::enable_if<HasDataAndSize<
  ...
  size_t AppendData(size_t max_elements, F&& setter) {
    ...
    assert(written_elements <= max_elements);
 ...
 private:
  ...
  void OnMovedFrom() {
    assert(!data_);  // Our heap block should have been stolen.
#if RTC_DCHECK_IS_ON
