...
class IceTransportInternal : public PacketTransportInternal {
 public:
  ...
  virtual IceTransportStandardState GetIceTransportState() const = 0;
  ...
  signal1<IceTransportInternal*> SignalGatheringState;

  // Handles sending and receiving of candidates.
  signal2<IceTransportInternal*, const Candidate&> SignalCandidateGathered;

  signal2<IceTransportInternal*, const IceCandidateErrorEvent&>
      SignalCandidateError;

  signal2<IceTransportInternal*, const Candidates&> SignalCandidatesRemoved;
  ...
  signal2<IceTransportInternal*, const Candidate&> SignalRouteChange;

  signal1<const CandidatePairChangeEvent&> SignalCandidatePairChanged;

  // Invoked when there is conflict in the ICE role between local and remote
  // agents.
  signal1<IceTransportInternal*> SignalRoleConflict;

  // Emitted whenever the transport state changed.
  // TODO(bugs.webrtc.org/9308): Remove once all uses have migrated to the new
  // IceTransportState.
  signal1<IceTransportInternal*> SignalStateChanged;

  // Emitted whenever the new standards-compliant transport state changed.
  signal1<IceTransportInternal*> SignalIceTransportStateChanged;

  // Invoked when the transport is being destroyed.
  signal1<IceTransportInternal*> SignalDestroyed;
};
