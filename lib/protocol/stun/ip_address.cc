...
std::string InterfaceAddress::ToString() const {
  ...
  if (family() == AF_INET6) result += "|flags:0x" + ToHex(ipv6_flags());
...
bool IPIsPrivateNetwork(const IPAddress& ip) {
  switch (ip.family()) {
    case AF_INET: { return IPIsPrivateNetworkV4(ip); }
    case AF_INET6: { return IPIsPrivateNetworkV6(ip); }
  }
  return false;
}
...
size_t HashIP(const IPAddress& ip) {
  switch (ip.family()) {
    case AF_INET: { return ip.ipv4_address().s_addr; }
    case AF_INET6: {
...
bool IPIsLinkLocal(const IPAddress& ip) {
  switch (ip.family()) {
    case AF_INET: { return IPIsLinkLocalV4(ip); }
    case AF_INET6: { return IPIsLinkLocalV6(ip); }
  }
  return false;
}
