...
bool MatchTypeNameWithIndexPattern(std::string_view network_name,
                                   std::string_view type_name) {
  if (!StartsWith(network_name, type_name)) {
    return false;
  }
  auto view = network_name.substr(type_name.size());
  return std::none_of(view.begin(), view.end(),
...
// webMdnsResponderInterface* NetworkManager::GetMdnsResponder() const {
//   return nullptr;
// }
...
void NetworkManagerBase::GetAnyAddressNetworks(NetworkList* networks) {
  if (!ipv4_any_address_network_) {
    ...
    ipv4_any_address_network_.reset(
        new Network("any", "any", ipv4_any_address, 0, ADAPTER_TYPE_ANY));
  ...
  if (!ipv6_any_address_network_) {
    ...
    ipv6_any_address_network_.reset(
        new Network("any", "any", ipv6_any_address, 0, ADAPTER_TYPE_ANY));
  ...
  if (*changed) {
    ...
    for (Network* network : networks_) {
      ...
      if (pref > 0) {
        ...
        LOG(ERROR) << "Too many network interfaces to handle!";
        break;
      }
    }
  }
...
Network* NetworkManagerBase::GetNetworkFromAddress(const IPAddress& ip) const {
  for (Network* network : networks_) {
    ...
    if (std::any_of(ips.begin(), ips.end(),
                    [&](const InterfaceAddress& existing_ip) {
...
void BasicNetworkManager::OnNetworksChanged() {
  assert(loop_);
  LOG(INFO) << "Network change was observed";
  UpdateNetworksOnce();
}
...
bool BasicNetworkManager::CreateNetworks(bool include_ignored,
  ...
  if (error != 0) {
    LOG(ERROR) << "getifaddrs failed to gather interface data: " << error;
    return false;
  }
...
unsigned int GetPrefix(PIP_ADAPTER_PREFIX prefixlist, const IPAddress& ip,
                       IPAddress* prefix) {
...
IPAddress BasicNetworkManager::QueryDefaultLocalAddress(int family) const {
  ...
  if (socket->Connect(SocketAddress(
    ...
    if (socket->GetError() != ENETUNREACH &&
      ...
      LOG(INFO) << "Connect failed with " << socket->GetError();
    }
    return IPAddress();
  }
...
void BasicNetworkManager::DumpNetworks() {
  ...
  NetworkList list;
  GetNetworks(&list);
  LOG(INFO) << "NetworkManager detected " << list.size() << " networks:";
  for (const Network* network : list) {
    LOG(INFO) << network->ToString() << ": " << network->description()
...
IPAddress Network::GetBestIP() const {
  ...
  for (const InterfaceAddress& ip : ips_) {
    ...
    if (ip.ipv6_flags() & IPV6_ADDRESS_FLAG_DEPRECATED) continue;
    ...
    if (ip.ipv6_flags() & IPV6_ADDRESS_FLAG_TEMPORARY) break;
  }
...
// webMdnsResponderInterface* Network::GetMdnsResponder() const {
