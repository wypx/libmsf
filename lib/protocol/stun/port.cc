...
const char* ProtoToString(ProtocolType proto) { return PROTO_NAMES[proto]; }

bool StringToProto(const char* value, ProtocolType* proto) {
  for (size_t i = 0; i <= PROTO_LAST; ++i) {
    if (EqualsIgnoreCase(PROTO_NAMES[i], value)) {
...
std::string Port::ComputeFoundation(const std::string& type,
  ...
  uint32_t crc = Crc32((uint8_t*)sb.str().c_str(), sb.str().size());
  return MSF::ToString(crc);
}
...
Port::Port(const std::string& type, PacketSocketFactory* factory,
           Network* network, const std::string& username_fragment,
...
void Port::Construct() {
  ...
  //                      MSG_DESTROY_IF_DEAD);
  LOG(INFO) << ToString() << ": Port created with network cost "
            << network_cost_;
}
...
Port::~Port() {
  ...
  for (uint32_t i = 0; i < list.size(); i++) delete list[i];
}

const std::string& Port::Type() const { return type_; }
Network* Port::network() const { return network_; }

IceRole Port::GetIceRole() const { return ice_role_; }

void Port::SetIceRole(IceRole role) { ice_role_ = role; }

void Port::SetIceTiebreaker(uint64_t tiebreaker) { tiebreaker_ = tiebreaker; }
uint64_t Port::IceTiebreaker() const { return tiebreaker_; }

bool Port::SharedSocket() const { return shared_socket_; }

void Port::SetIceParameters(int component, const std::string& username_fragment,
                            const std::string& password) {
...
const std::vector<Candidate>& Port::Candidates() const { return candidates_; }
...
void Port::AddAddress(const SocketAddress& address,
                      ...
                      const std::string& protocol,
                      const std::string& relay_protocol,
                      const std::string& tcptype, const std::string& type,
                      uint32_t type_preference, uint32_t relay_preference,
                      const std::string& url, bool is_final) {
...
bool Port::MaybeObfuscateAddress(Candidate* c, const std::string& type,
                                 bool is_final) {
  ...
  auto weak_ptr = weak_factory_.get();
  auto callback =
      [weak_ptr, copy, is_final](const IPAddress& addr, const std::string& name) mutable {
    assert(copy.address().ipaddr() == addr);
...
void Port::OnReadPacket(const char* data, size_t size,
                        const SocketAddress& addr, ProtocolType proto) {
  ...
  if (!GetStunMessage(data, size, addr, &msg, &remote_username)) {
  ...
  } else if (msg->type() == kStunMessageBindingRequest) {
    LOG(INFO) << "Received " << StunMethodToString(msg->type())
  ...
  } else if (msg->type() == kStunGoodPingRequest) {
    // This is a PING sent to a connection that was destroyed.
    // Send back that this is the case and a authenticated BINDING
    // is needed.
    SendBindingErrorResponse(msg.get(), addr, kStunBadRequest,
                             StunErrorToString(kStunBadRequest));
  } else {
    // NOTE(tschmelcher): kStunMessageBindingResponse is benign. It occurs if we
    ...
    // does not handle.
    if (msg->type() != kStunMessageBindingResponse &&
        msg->type() != kStunGoodPingResponse &&
        msg->type() != kStunGoodPingErrorResponse) {
      LOG(ERROR) << ToString()
...
bool Port::GetStunMessage(const char* data, size_t size,
  ...
  // Except kStunGoodPingRequest/RESPONSE that does not send fingerprint.
  int types[] = {kStunGoodPingRequest, kStunGoodPingResponse,
                 kStunGoodPingErrorResponse};
  ...
  if (stun_msg->type() == kStunMessageBindingRequest) {
    // Check for the presence of USERNAME and MESSAGE-INTEGRITY (if ICE) first.
    // If not present, fail with a 400 Bad Request.
    if (!stun_msg->GetByteString(kStunAttrUsername) ||
        !stun_msg->GetByteString(kStunAttrMessageIntegrity)) {
      LOG(ERROR) << ToString() << ": Received "
                 << StunMethodToString(stun_msg->type())
                 << " without username/M-I from: " << addr.ToSensitiveString();
      SendBindingErrorResponse(stun_msg.get(), addr, kStunBadRequest,
                               StunErrorToString(kStunBadRequest));
      return true;
    }
    ...
    if (!ParseStunUsername(stun_msg.get(), &local_ufrag, &remote_ufrag) ||
      ...
      LOG(ERROR) << ToString() << ": Received "
                 << StunMethodToString(stun_msg->type())
                 << " with bad local username " << local_ufrag << " from "
                 << addr.ToSensitiveString();
      SendBindingErrorResponse(stun_msg.get(), addr, kStunNotAuthorized,
  ...
  } else if ((stun_msg->type() == kStunMessageBindingResponse) ||
             (stun_msg->type() == kStunMessageBindingErrorResponse)) {
    if (stun_msg->type() == kStunMessageBindingErrorResponse) {
      if (const StunErrorCodeAttribute* error_code = stun_msg->GetErrorCode()) {
        ...
        LOG(ERROR) << ToString() << ": Received "
                   << StunMethodToString(stun_msg->type())
                   << " without a error code from " << addr.ToSensitiveString();
        return true;
      }
    }
    ...
    if (!unknown_attributes.empty()) {
      LOG(ERROR) << ToString() << ": Discarding STUN response due to unknown "
                                  "comprehension-required attribute";
      return true;
    }
    // NOTE: Username should not be used in verifying response messages.
    out_username->clear();
  } else if (stun_msg->type() == kStunMessageBindingIndication) {
    LOG(TRACE) << ToString() << ": Received "
    ...
    if (!unknown_attributes.empty()) {
      LOG(ERROR) << ToString() << ": Discarding STUN indication due to "
  ...
  } else if (stun_msg->type() == kStunGoodPingRequest) {
    ...
    LOG(TRACE) << ToString() << ": Received "
               ...
               << addr.ToSensitiveString();
    out_username->clear();
  } else if (stun_msg->type() == kStunGoodPingResponse ||
             stun_msg->type() == kStunGoodPingErrorResponse) {
    ...
    LOG(ERROR) << ToString() << ": Received STUN packet with invalid type ("
               << stun_msg->type() << ") from " << addr.ToSensitiveString();
    return true;
  }
...
bool Port::ParseStunUsername(const StunMessage* stun_msg,
  ...
  const StunByteStringAttribute* username_attr =
      stun_msg->GetByteString(kStunAttrUsername);
  if (username_attr == NULL) return false;
...
bool Port::MaybeIceRoleConflict(const SocketAddress& addr, IceMessage* stun_msg,
  ...
  const StunUInt64Attribute* stun_attr =
      stun_msg->GetUInt64(kStunAttrICEControlling);
  if (stun_attr) {
  ...
  stun_attr = stun_msg->GetUInt64(kStunAttrICEControlled);
  if (stun_attr) {
    ...
    case ICEROLE_CONTROLLING:
      if (ICEROLE_CONTROLLING == remote_ice_role) {
        if (remote_tiebreaker >= tiebreaker_) {
          ...
          SendBindingErrorResponse(stun_msg, addr, kStunRoleConflict,
                                   StunErrorToString(kStunRoleConflict));
          ret = false;
        }
      }
      break;
...
bool Port::HandleIncomingPacket(AsyncPacketSocket* socket, const char* data,
...
void Port::SendBindingErrorResponse(StunMessage* request,
  ...
  assert(request->type() == kStunMessageBindingRequest ||
         request->type() == kStunGoodPingRequest);

  // Fill in the response message.
  StunMessage response;
  if (request->type() == kStunMessageBindingRequest) {
    response.SetType(kStunMessageBindingErrorResponse);
  } else {
    response.SetType(kStunGoodPingErrorResponse);
  }
  ...
  if (error_code != kStunBadRequest && error_code != kStunNotAuthorized &&
      request->type() != kStunGoodPingRequest) {
    if (request->type() == kStunMessageBindingRequest) {
  ...
  LOG(INFO) << ToString() << ": Sending STUN "
            << StunMethodToString(response.type()) << ": reason=" << reason
...
void Port::SendUnknownAttributesErrorResponse(
  ...
  auto error_attr = StunAttribute::CreateErrorCode();
  error_attr->SetCode(kStunUnknownAttribute);
  error_attr->SetReason(StunErrorToString(kStunUnknownAttribute));
  response.AddAttribute(std::move(error_attr));
  ...
  SendTo(buf.Data(), buf.Length(), addr, options, false);
  LOG(ERROR) << ToString()
             << ": Sending STUN binding error: reason=" << kStunUnknownAttribute
             << " to " << addr.ToSensitiveString();
}
...
void Port::OnNetworkTypeChanged(const Network* network) {
...
void Port::UpdateNetworkCost() {
  ...
  LOG(INFO) << "Network cost changed from " << network_cost_ << " to "
            << new_cost
...
void Port::EnablePortPackets() { enable_port_packets_ = true; }
...
void Port::Destroy() {
  ...
  LOG(INFO) << ToString() << ": Port deleted";
