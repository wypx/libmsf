...
struct IceCandidateErrorEvent {
  ...
  IceCandidateErrorEvent(std::string address, int port, std::string url,
                         int error_code, std::string error_text)
...
class Port : public PortInterface, public MessageHandler, public has_slots<> {
 public:
  ...
  Port(const std::string& type, PacketSocketFactory* factory, Network* network,
  ...
  PacketSocketFactory* socket_factory() const { return factory_; }
  void set_socket_factory(PacketSocketFactory* factory) { factory_ = factory; }
  ...
  void SetIceParameters(int component, const std::string& username_fragment,
                        const std::string& password);

  // Fired when candidates are discovered by the port. When all candidates
  // are discovered that belong to port SignalAddressReady is fired.
  signal2<Port*, const Candidate&> SignalCandidateReady;
  // Provides all of the above information in one handy object.
  const std::vector<Candidate>& Candidates() const override;
  // Fired when candidate discovery failed using certain server.
  signal2<Port*, const IceCandidateErrorEvent&> SignalCandidateError;

  // SignalPortComplete is sent when port completes the task of candidates
  // allocation.
  signal1<Port*> SignalPortComplete;
  ...
  virtual bool HandleIncomingPacket(AsyncPacketSocket* socket, const char* data,
                                    size_t size,
  ...
  virtual bool CanHandleIncomingPacketsFrom(const SocketAddress& remote_addr)
      const;

  // Sends a response error to the given request.
  void SendBindingErrorResponse(StunMessage* request, const SocketAddress& addr,
                                int error_code,
  ...
  //////////////////////////////////
  void set_proxy(const std::string& user_agent, const ProxyInfo& proxy) {
    user_agent_ = user_agent;
    proxy_ = proxy;
  }
  const std::string& user_agent() { return user_agent_; }
  const ProxyInfo& proxy() { return proxy_; }
  ...
  bool MaybeIceRoleConflict(const SocketAddress& addr, IceMessage* stun_msg,
  ...
  virtual void OnSentPacket(AsyncPacketSocket* socket,
                            const SentPacket& sent_packet) = 0;
  ...
  // EventLoop* event_loop() { return factory_->event_loop(); }

 protected:
  enum {
    MSG_DESTROY_IF_DEAD = 0,
    MSG_FIRST_AVAILABLE
  };
  ...
  void AddAddress(const SocketAddress& address,
                  ...
                  const std::string& protocol,
                  const std::string& relay_protocol, const std::string& tcptype,
                  const std::string& type, uint32_t type_preference,
                  uint32_t relay_preference, const std::string& url,
                  bool is_final);
  ...
  virtual DiffServCodePoint StunDscpValue() const;
 ...
 private:
  ...
  void OnNetworkTypeChanged(const Network* network);
  ...
  std::weak_ptr<Port> weak_factory_;

  bool MaybeObfuscateAddress(Candidate* c, const std::string& type,
                             bool is_final);
