...
RelayServerConfig::RelayServerConfig(const std::string& address, int port,
                                     const std::string& username,
                                     const std::string& password,
                                     ProtocolType proto)
    : RelayServerConfig(SocketAddress(address, port), username, password,
                        proto) {}
...
RelayServerConfig::RelayServerConfig(const std::string& address, int port,
    ...
    : RelayServerConfig(address, port, username, password,
...
PortAllocatorSession::PortAllocatorSession(const std::string& content_name,
  ...
  assert(ice_ufrag.empty() == ice_pwd.empty());
}

PortAllocatorSession::~PortAllocatorSession() = default;

bool PortAllocatorSession::IsCleared() const { return false; }

bool PortAllocatorSession::IsStopped() const { return false; }

uint32_t PortAllocatorSession::generation() { return generation_; }
...
PortAllocator::~PortAllocator() { CheckRunOnValidThreadIfInitialized(); }
...
bool PortAllocator::SetConfiguration(
    ...
    const std::vector<RelayServerConfig>& turn_servers, int candidate_pool_size,
    bool prune_turn_ports, TurnCustomizer* turn_customizer,
...
bool PortAllocator::SetConfiguration(
    const ServerAddresses& stun_servers,
    const std::vector<RelayServerConfig>& turn_servers, int candidate_pool_size,
    PortPrunePolicy turn_port_prune_policy, TurnCustomizer* turn_customizer,
    const std::optional<int>& stun_candidate_keepalive_interval) {
  ...
  // the network thread.
  // assert(candidate_pool_size == 0 || thread_checker_.IsCurrent());
  ...
  if (candidate_pool_size < 0) {
    LOG(ERROR) << "Can't set negative pool size.";
    return false;
  }
...
std::unique_ptr<PortAllocatorSession> PortAllocator::TakePooledSession(
    ...
    const std::string& ice_ufrag, const std::string& ice_pwd) {
  CheckRunOnValidThreadAndInitialized();
  assert(!ice_ufrag.empty());
  assert(!ice_pwd.empty());
