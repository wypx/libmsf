...
enum class IceTransportStandardState {
  kNew,
  kChecking,
  kConnected,
  kCompleted,
...
enum {
  // Disable local UDP ports. This doesn't impact how connect to relay
  // servers.
  PORTALLOCATOR_DISABLE_UDP = 0x01,
  PORTALLOCATOR_DISABLE_STUN = 0x02,
  PORTALLOCATOR_DISABLE_RELAY = 0x04,
  // Disable local TCP ports. This doesn't impact how connect to relay
  // servers.
...
struct RelayServerConfig {
  RelayServerConfig();
  RelayServerConfig(const SocketAddress& address, const std::string& username,
                    const std::string& password, ProtocolType proto);
  RelayServerConfig(const std::string& address, int port,
                    const std::string& username, const std::string& password,
                    ProtocolType proto);
  // Legacy constructor where "secure" and PROTO_TCP implies PROTO_TLS.
  RelayServerConfig(const std::string& address, int port,
                    const std::string& username, const std::string& password,
                    ProtocolType proto, bool secure);
...
class PortAllocatorSession : public has_slots<> {
 public:
  // Content name passed in mostly for logging and debugging.
  PortAllocatorSession(const std::string& content_name, int component,
  ...
  // ready(pairable).
  signal2<PortAllocatorSession*, const std::vector<PortInterface*>&>
      SignalPortsPruned;
  signal2<PortAllocatorSession*, const std::vector<Candidate>&>
      SignalCandidatesReady;
  signal2<PortAllocatorSession*, const IceCandidateErrorEvent&>
  ...
  signal2<PortAllocatorSession*, IceRegatheringReason> SignalIceRegathering;
 ...
 private:
  void SetIceParameters(const std::string& content_name, int component,
...
class PortAllocator : public has_slots<> {
 public:
  PortAllocator();
  ...
  bool SetConfiguration(
      ...
      int candidate_pool_size, bool prune_turn_ports,
  ...
  bool SetConfiguration(
      const ServerAddresses& stun_servers,
      const std::vector<RelayServerConfig>& turn_servers,
      int candidate_pool_size, PortPrunePolicy turn_port_prune_policy,
      TurnCustomizer* turn_customizer = nullptr,
      const std::optional<int>& stun_candidate_keepalive_interval =
          std::nullopt);
  ...
  //
  // Specifically for the session taken by the ICE transport, currently do
  ...
  signal2<uint32_t /* prev_filter */, uint32_t /* cur_filter */>
      SignalCandidateFilterChanged;

 protected:
  virtual PortAllocatorSession* CreateSessionInternal(
      const std::string& content_name, int component,
      const std::string& ice_ufrag, const std::string& ice_pwd) = 0;
  ...
  void CheckRunOnValidThreadIfInitialized() const {
    // assert(!initialized_ || thread_checker_.IsCurrent());
  }
