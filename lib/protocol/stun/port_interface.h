...
class PortInterface {
 public:
  ...
  virtual int SendTo(const void* data, size_t size, const SocketAddress& addr,
                     const PacketOptions& options, bool payload) = 0;

  // Indicates that we received a successful STUN binding request from an
  // address that doesn't correspond to any current connection.  To turn this
  // into a real connection, call CreateConnection.
  signal6<PortInterface*, const SocketAddress&, ProtocolType, IceMessage*,
  ...
  signal4<PortInterface*, const char*, size_t, const SocketAddress&>
      SignalReadPacket;

  // Emitted each time a packet is sent on this port.
  signal1<const SentPacket&> SignalSentPacket;
