...
#if PSEUDO_KEEPALIVE
// !?! Rethink these times
const uint32_t IDLE_PING = 20 * 1000;  // 20 seconds (note: WinXP SP2 firewall
...
// retransmit segments
      ...
      LOG(INFO) << "timeout retransmit (rto: " << m_rx_rto
      ...
      // LOG(INFO) << "m_ssthresh: " << m_ssthresh << "  nInFlight: " <<
...
uint32_t PseudoTcp::GetCongestionWindow() const { return m_cwnd; }

uint32_t PseudoTcp::GetBytesInFlight() const { return m_snd_nxt - m_snd_una; }
...
uint32_t PseudoTcp::GetRoundTripTimeEstimateMs() const { return m_rx_srtt; }
...
int PseudoTcp::Recv(char* buffer, size_t len) {
  ...
  size_t read = 0;
  StreamResult result = m_rbuf.Read(buffer, len, &read, NULL);
...
void PseudoTcp::Close(bool force) {
  LOG(TRACE) << "(" << (force ? "true" : "false") << ")";
  m_shutdown = force ? SD_FORCEFUL : SD_GRACEFUL;
}

int PseudoTcp::GetError() { return m_error; }
...
IPseudoTcpNotify::WriteResult PseudoTcp::packet(uint32_t seq, uint8_t flags,
  ...
  if ((wres != IPseudoTcpNotify::WR_SUCCESS) && (0 != len)) return wres;
...
bool PseudoTcp::clock_check(uint32_t now, long& nTimeout) {
  if (m_shutdown == SD_FORCEFUL) return false;
  ...
  if (seg.flags & FLAG_CTL) {
    if (seg.len == 0) {
      ...
      if (m_state == TCP_LISTEN) {
        ...
        LOG(INFO) << "State: TCP_SYN_RECEIVED";
        // m_notify->associate(addr);
        queueConnectMessage();
      } else if (m_state == TCP_SYN_SENT) {
        m_state = TCP_ESTABLISHED;
        LOG(INFO) << "State: TCP_ESTABLISHED";
        adjustMTU();
        if (m_notify) {
  ...
  if ((seg.ack > m_snd_una) && (seg.ack <= m_snd_nxt)) {
    ...
    if (seg.tsecr) {
      ...
      if (rtt >= 0) {
        ...
        uint32_t tmp_rto = std::max(1U, 4 * m_rx_rttvar);
        if (tmp_rto < MIN_RTO) {
          m_rx_rto = MIN_RTO;
        } else {
          if (tmp_rto >= MAX_RTO) {
...
#if _DEBUGMSG >= _DBG_VERBOSE
        LOG(INFO) << "rtt: " << rtt << "  srtt: " << m_rx_srtt
                  << "  rto: " << m_rx_rto;
#endif  // _DEBUGMSG
      } else {
        LOG(WARNING) << "rtt < 0";
...
#if _DEBUGMSG >= _DBG_NORMAL
        LOG(INFO) << "exit recovery";
#endif  // _DEBUGMSG
        m_dup_acks = 0;
      } else {
#if _DEBUGMSG >= _DBG_NORMAL
        LOG(INFO) << "recovery retransmit";
#endif  // _DEBUGMSG
        if (!transmit(m_slist.begin(), now)) {
          closedown(ECONNABORTED);
          return false;
...
#if _DEBUGMSG >= _DBG_NORMAL
        LOG(INFO) << "Saving " << seg.len << " bytes (" << seg.seq << " -> "
                  << seg.seq + seg.len << ")";
#endif  // _DEBUGMSG
        RSegment rseg;
        rseg.seq = seg.seq;
...
#if _DEBUGMSG >= _DBG_NORMAL
          LOG(INFO) << "Recovered " << nAdjust << " bytes (" << m_rcv_nxt
                    << " -> " << m_rcv_nxt + nAdjust << ")";
#endif  // _DEBUGMSG
...
bool PseudoTcp::transmit(const SList::iterator& seg, uint32_t now) {
  if (seg->xmit >= ((m_state == TCP_ESTABLISHED) ? 15 : 30)) {
    LOG(TRACE) << "too many retransmits";
    return false;
  }
  ...
  while (true) {
    ...
    if (wres == IPseudoTcpNotify::WR_SUCCESS) break;

    if (wres == IPseudoTcpNotify::WR_FAIL) {
      LOG(TRACE) << "packet failed";
      return false;
    }
...
#if _DEBUGMSG >= _DBG_NORMAL
    LOG(INFO) << "Adjusting mss to " << m_mss << " bytes";
#endif  // _DEBUGMSG
  }

  if (nTransmit < seg->len) {
    LOG(TRACE) << "mss reduced to " << m_mss;
...
void PseudoTcp::attemptSend(SendFlags sflags) {
  ...
  while (true) {
    ...
    if (bFirst) {
      ...
      bFirst = false;
      LOG(INFO) << "[cwnd: " << m_cwnd << "  nWindow: " << nWindow
                << "  nInFlight: " << nInFlight
...
void PseudoTcp::disableWindowScale() { m_support_wnd_scale = false; }
