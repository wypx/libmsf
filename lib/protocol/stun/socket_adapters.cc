...
BufferedReadAdapter::~BufferedReadAdapter() { delete[] buffer_; }
...
void BufferedReadAdapter::BufferInput(bool on) { buffering_ = on; }
...
void AsyncSSLSocket::OnConnectEvent(AsyncSocket* socket) {
  ...
  assert(sizeof(kSslClientHello) == res);
}

void AsyncSSLSocket::ProcessInput(char* data, size_t* len) {
  if (*len < sizeof(kSslServerHello)) return;
  ...
  *len -= sizeof(kSslServerHello);
  ...
  if (remainder) SignalReadEvent(this);
}
  ...
  if (len == 0) {
    if (state_ == PS_TUNNEL_HEADERS) {
      ...
      if (!unknown_mechanisms_.empty()) {
        LOG(ERROR) << "Unsupported authentication methods: "
                          << unknown_mechanisms_;
      }
...
SocketAddress AsyncSocksProxySocket::GetRemoteAddress() const { return dest_; }
...
void AsyncSocksProxySocket::OnConnectEvent(AsyncSocket* socket) { SendHello(); }
...
void AsyncSocksProxySocket::ProcessInput(char* data, size_t* len) {
  ...
  if (state_ == SS_HELLO) {
    uint8_t ver, method;
    if (!response.ReadUInt8(&ver) || !response.ReadUInt8(&method)) return;
    ...
    uint8_t ver, status;
    if (!response.ReadUInt8(&ver) || !response.ReadUInt8(&status)) return;
  ...
  *len = response.Length();
  ...
  // FIX: if SignalConnect causes the socket to be destroyed, we are in trouble
  if (remainder) SignalReadEvent(this);  // TODO: signal this??
}
