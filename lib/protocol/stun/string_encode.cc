...
size_t hex_decode_with_delimiter(char* cbuffer, size_t buflen,
                                 ...
                                 char delimiter) {
  assert(cbuffer);  // TODO(kwiberg): estimate output size
  ...
  while (srcpos < srclen) {
    ...
    if (delimiter && (srclen - srcpos) > 1) {
      if (source[srcpos] != delimiter) return 0;
      ++srcpos;
    }
  }
...
std::string ToString(const char* const s) { return std::string(s); }
std::string ToString(const std::string s) { return s; }

std::string ToString(const short s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%hd", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const unsigned short s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%hu", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const int s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%d", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const unsigned int s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%u", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const long int s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%ld", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const unsigned long int s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%lu", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const long long int s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%lld", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
std::string ToString(const unsigned long long int s) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%llu", s);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
...
std::string ToString(const double d) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%g", d);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
...
std::string ToString(const long double d) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%Lg", d);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
...
std::string ToString(const void* const p) {
  char buf[32];
  const size_t len = std::snprintf(&buf[0], arraysize(buf), "%p", p);
  assert(len <= arraysize(buf));
  return std::string(&buf[0], len);
}
