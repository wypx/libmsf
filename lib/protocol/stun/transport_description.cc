...
RTCError ValidateIceUfrag(string_view raw_ufrag) {
  ...
  if (!c_all_of(raw_ufrag, IsIceChar)) {
    return RTCError(
...
RTCError ValidateIcePwd(string_view raw_pwd) {
...
#if 0
RTCErrorOr<IceParameters> IceParameters::Parse(string_view raw_ufrag,
                                               string_view raw_pwd) {
...
bool StringToConnectionRole(const std::string& role_str, ConnectionRole* role) {
  ...
  for (size_t i = 0; i < arraysize(roles); ++i) {
    if (EqualsIgnoreCase(roles[i], role_str)) {
...
//     const std::string& ice_ufrag, const std::string& ice_pwd, IceMode
// ice_mode,
//     ConnectionRole role, const rtc::SSLFingerprint* identity_fingerprint)
