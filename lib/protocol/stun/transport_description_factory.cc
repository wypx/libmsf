...
#include "transport_description_factory.h"
...
std::unique_ptr<TransportDescription> TransportDescriptionFactory::CreateAnswer(
  ...
  if (!offer) {
    LOG(WARNING) << "Failed to create TransportDescription answer "
...
#if 0
  if (!certificate_) {
    LOG(ERROR) << "Cannot create identity digest with no certificate";
    return false;
  }
