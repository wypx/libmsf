...
// kStunAllocationMismatch error per rfc5766.
...
class TurnCreatePermissionRequest : public StunRequest, public has_slots<> {
 public:
...
class TurnChannelBindRequest : public StunRequest, public has_slots<> {
 public:
...
class TurnEntry : public has_slots<> {
 public:
  enum BindState {
    STATE_UNBOUND,
    STATE_BINDING,
  ...
  TurnEntry(TurnPort* port, int channel_id, const SocketAddress& ext_addr,
            const std::string remote_ufrag);
  ...
  std::optional<int64_t> destruction_timestamp() {
  ...
  signal1<TurnEntry*> SignalDestroyed;
 ...
 private:
  ...
  std::optional<int64_t> destruction_timestamp_;

  std::string remote_ufrag_;
};

TurnPort::TurnPort(EventLoop* loop, PacketSocketFactory* factory,
                   Network* network, AsyncPacketSocket* socket,
                   const std::string& username, const std::string& password,
                   const ProtocolAddress& server_address,
                   const RelayCredentials& credentials, int server_priority,
                   const std::string& origin, TurnCustomizer* customizer)
    : Port(/*loop,*/ RELAY_PORT_TYPE, factory, network, username, password),
...
TurnPort::TurnPort(EventLoop* loop, PacketSocketFactory* factory,
                   ...
                   webTurnCustomizer* customizer,
                   SSLCertificateVerifier* tls_cert_verifier)
    : Port(/*loop,*/RELAY_PORT_TYPE, factory, network, min_port, max_port,
           username, password),
      server_address_(server_address),
...
ProtocolType TurnPort::GetProtocol() const { return server_address_.proto; }

TlsCertPolicy TurnPort::GetTlsCertPolicy() const { return tls_cert_policy_; }
...
void TurnPort::PrepareAddress() {
  if (credentials_.username.empty() || credentials_.password.empty()) {
    LOG(ERROR) << "Allocation can't be started without setting the"
                  " TURN server credentials for the user.";
    OnAllocateError(kStunNotAuthorized, "Missing TURN server credentials.");
    return;
  }
  ...
  if (server_address_.address.IsUnresolvedIP()) {
    ...
    if (!IsCompatibleAddress(server_address_.address)) {
      LOG(ERROR) << "IP address family does not match. server: "
                 << server_address_.address.family()
                 << " local: " << network()->GetBestIP().family();
      OnAllocateError(kStunGlobalFailure, "IP address family does not match.");
      return;
    }
    ...
    LOG(INFO) << ToString() << ": Trying to connect to TURN server via "
              << ProtoToString(server_address_.proto) << " @ "
              << server_address_.address.ToSensitiveString();
    if (!CreateTurnClientSocket()) {
      LOG(ERROR) << "Failed to create TURN client socket";
      OnAllocateError(kStunServerNotReachable,
...
void TurnPort::OnSocketConnect(AsyncPacketSocket* socket) {
  ...
  auto ips = network()->GetIPs();
  if (std::none_of(ips.begin(), ips.end(),
                   [socket_address](const InterfaceAddress& addr) {
        return socket_address.ipaddr() == addr;
      })) {
    if (socket->GetLocalAddress().IsLoopbackIP()) {
      LOG(TRACE) << "Socket is bound to the address:"
  ...
  LOG(INFO) << "TurnPort connected to "
            << socket->GetRemoteAddress().ToSensitiveString() << " using tcp.";
  SendRequest(new TurnAllocateRequest(this), 0);
}

void TurnPort::OnSocketClose(AsyncPacketSocket* socket, int error) {
  LOG(TRACE) << ToString()
             << ": Connection with server failed with error: " << error;
...
void TurnPort::OnAllocateMismatch() {
  if (allocate_mismatch_retries_ >= MAX_ALLOCATE_MISMATCH_RETRIES) {
    LOG(TRACE) << ToString() << ": Giving up on the port after "
               << allocate_mismatch_retries_
               << " retries for kStunAllocationMismatch";
    OnAllocateError(kStunAllocationMismatch,
                    "Maximum retries reached for allocation mismatch.");
    return;
  }

  LOG(INFO) << ToString() << ": Allocating a new socket after "
                             "kStunAllocationMismatch, retry: "
...
Connection* TurnPort::CreateConnection(const Candidate& remote_candidate,
  ...
  // request.
  if (EndsWith(remote_candidate.address().hostname(), LOCAL_TLD)) {
    return nullptr;
  }
...
int TurnPort::GetError() { return error_; }

int TurnPort::SendTo(const void* data, size_t size, const SocketAddress& addr,
                     const PacketOptions& options, bool payload) {
  // Try to find an entry for this specific address; we should have one.
  TurnEntry* entry = FindEntry(addr);
  if (!entry) {
    LOG(ERROR) << "Did not find the TurnEntry for address "
...
bool TurnPort::HandleIncomingPacket(AsyncPacketSocket* socket, const char* data,
                                    size_t size,
  ...
  if (remote_addr != server_address_.address) {
    LOG(TRACE) << ToString()
               ...
               << remote_addr.ToSensitiveString() << " server_address_: "
               << server_address_.address.ToSensitiveString();
    return false;
  }
  ...
  if (size < TURN_CHANNEL_HEADER_SIZE) {
    LOG(TRACE) << ToString() << ": Received TURN message that was too short";
    return false;
  }
  ...
  if (msg_type == kTurnDataSendIndication) {
    HandleDataIndication(data, size, packet_time_us);
    return true;
  }

  if (SharedSocket() && (msg_type == kStunMessageBindingResponse ||
                         msg_type == kStunMessageBindingErrorResponse)) {
    LOG(TRACE) << ToString()
  ...
  if (IsStunSuccessResponseType(msg_type) &&
    ...
    LOG(TRACE) << ToString() << ": Received TURN message with invalid "
...
void TurnPort::OnReadPacket(AsyncPacketSocket* socket, const char* data,
...
bool TurnPort::SetAlternateServer(const SocketAddress& address) {
  ...
  if (iter != attempted_server_addresses_.end()) {
    LOG(TRACE) << ToString() << ": Redirection to ["
  ...
  if (!IsCompatibleAddress(address)) {
    LOG(TRACE) << "Server IP address family does not match with "
  ...
  LOG(INFO) << ToString() << ": Redirecting from TURN server ["
            << server_address_.address.ToSensitiveString()
...
void TurnPort::ResolveTurnAddress(const SocketAddress& address) {
  ...
  LOG(INFO) << ToString() << ": Starting TURN host lookup for "
            << address.ToSensitiveString();
...
void TurnPort::OnResolveResult(AsyncResolverInterface* resolver) {
#if 0
  ...
  if (resolver_->GetError() != 0 ||
      !resolver_->GetResolvedAddress(network()->GetBestIP().family(),
                                     &resolved_address)) {
    LOG(TRACE) << ToString() << ": TURN host lookup received error "
               << resolver_->GetError();
...
void TurnPort::OnSendStunPacket(const void* data, size_t size,
  ...
  if (Send(data, size, options) < 0) {
    LOG(ERROR) << ToString() << ": Failed to send TURN message, error: "
               << socket_->GetError();
  }
}
...
void TurnPort::OnAllocateError(int error_code, const std::string& reason) {
  ...
  // creation.
  // loop()->Post(RTC_FROM_HERE, this, MSG_ALLOCATE_ERROR);
...
void TurnPort::OnRefreshError() {
  ...
  // loop()->Post(RTC_FROM_HERE, this, MSG_REFRESH_ERROR);
}
...
DiffServCodePoint TurnPort::StunDscpValue() const { return stun_dscp_value_; }

void TurnPort::OnMessage(Message* message) {
...
void TurnPort::OnAllocateRequestTimeout() {
  OnAllocateError(kStunServerNotReachable, "TURN allocate request timed out.");
}
...
void TurnPort::HandleDataIndication(const char* data, size_t size,
  ...
  TurnMessage msg;
  if (!msg.Read(&buf)) {
    LOG(TRACE) << ToString() << ": Received invalid TURN data indication";
    return;
  }

  // Check mandatory attributes.
  const StunAddressAttribute* addr_attr =
      msg.GetAddress(kStunAttrXorPeerAddress);
  if (!addr_attr) {
    LOG(TRACE) << ToString() << ": Missing kStunAttrXorPeerAddress attribute "
                                "in data indication.";
    return;
  }

  const StunByteStringAttribute* data_attr = msg.GetByteString(kStunAttrData);
  if (!data_attr) {
    LOG(TRACE) << ToString() << ": Missing kStunAttrData attribute in "
                                "data indication.";
    return;
  }
...
void TurnPort::HandleChannelData(int channel_id, const char* data, size_t size,
                                 int64_t packet_time_us) {
  ...
  if (len > size - TURN_CHANNEL_HEADER_SIZE) {
    LOG(TRACE) << ToString() << ": Received TURN channel data message with "
...
bool TurnPort::ScheduleRefresh(uint32_t lifetime) {
  ...
  if (lifetime < 2 * 60) {
    ...
    // for half lifetime.
    LOG(TRACE) << ToString()
               << ": Received response with short lifetime: " << lifetime
               << " seconds.";
    ...
    LOG(TRACE) << ToString()
               << ": Received response with long lifetime: " << lifetime
               << " seconds.";
  ...
  LOG(INFO) << ToString() << ": Scheduled refresh in " << delay << "ms.";
  return true;
}
...
void TurnPort::AddRequestAuthInfo(StunMessage* msg) {
  ...
  msg->AddAttribute(std::make_unique<StunByteStringAttribute>(
      kStunAttrUsername, credentials_.username));
  msg->AddAttribute(
      std::make_unique<StunByteStringAttribute>(kStunAttrRealm, realm_));
  msg->AddAttribute(
      std::make_unique<StunByteStringAttribute>(kStunAttrNonce, nonce_));
  const bool success = msg->AddMessageIntegrity(hash());
  assert(success);
}

int TurnPort::Send(const void* data, size_t len, const PacketOptions& options) {
...
bool TurnPort::UpdateNonce(StunMessage* response) {
  ...
  const StunByteStringAttribute* realm_attr =
      response->GetByteString(kStunAttrRealm);
  if (!realm_attr) {
    LOG(ERROR) << "Missing kStunAttrRealm attribute in "
  ...
  const StunByteStringAttribute* nonce_attr =
      response->GetByteString(kStunAttrNonce);
  if (!nonce_attr) {
    LOG(ERROR) << "Missing kStunAttrNonce attribute in "
                  "stale nonce error response.";
    return false;
  }
...
bool TurnPort::CreateOrRefreshEntry(const SocketAddress& addr,
                                    int channel_number) {
...
bool TurnPort::CreateOrRefreshEntry(const SocketAddress& addr,
  ...
  if (entry == nullptr) {
    ...
    //     LOG(INFO) << ToString() << ": remote ufrag updated."
...
void TurnPort::ScheduleEntryDestruction(TurnEntry* entry) {
  assert(!entry->destruction_timestamp().has_value());
  int64_t timestamp = TimeMillis();
  entry->set_destruction_timestamp(timestamp);
  // invoker_.AsyncInvokeDelayed<void>(
  //     RTC_FROM_HERE, loop(),
  //     Bind(&TurnPort::DestroyEntryIfNotCancelled, this, entry, timestamp),
  //     TURN_PERMISSION_TIMEOUT);
}

bool TurnPort::SetEntryChannelId(const SocketAddress& address, int channel_id) {
...
bool TurnPort::TurnCustomizerAllowChannelData(const void* data, size_t size,
                                              bool payload) {
  if (turn_customizer_ == nullptr) {
    return true;
  }
...
TurnAllocateRequest::TurnAllocateRequest(TurnPort* port)
    : StunRequest(loop_, new TurnMessage()), port_(port) {}

void TurnAllocateRequest::Prepare(StunMessage* request) {
  // Create the request as indicated in RFC 5766, Section 6.1.
  request->SetType(kStunAllocateRequst);
  auto transport_attr =
      StunAttribute::CreateUInt32(kStunAttrRequestedTransport);
...
void TurnAllocateRequest::OnResponse(StunMessage* response) {
  LOG(INFO) << port_->ToString()
            << ": TURN allocate requested successfully, id=" << hex_encode(id())
            << ", code=0"  // Makes logging easier to parse.
               ", rtt=" << Elapsed();

  // Check mandatory attributes as indicated in RFC5766, Section 6.3.
  const StunAddressAttribute* mapped_attr =
      response->GetAddress(kStunAttrXorMappedAddress);
  if (!mapped_attr) {
    LOG(TRACE) << port_->ToString() << ": Missing kStunAttrXorMappedAddress "
  ...
  const StunAddressAttribute* relayed_attr =
      response->GetAddress(kStunAttrXorRelayedAddress);
  if (!relayed_attr) {
    LOG(TRACE) << port_->ToString() << ": Missing kStunAttrXorRelayedAddress "
                                       "attribute in allocate success response";
    return;
  }

  const StunUInt32Attribute* lifetime_attr =
      response->GetUInt32(kStunAttrLifetime);
  if (!lifetime_attr) {
    LOG(TRACE) << port_->ToString()
               << ": Missing kStunAttrLifetime attribute in "
...
void TurnAllocateRequest::OnErrorResponse(StunMessage* response) {
  ...
  switch (error_code) {
    case kStunNotAuthorized:  // Unauthrorized.
      OnAuthChallenge(response, error_code);
      break;
    case kStunTryAlteate:
      OnTryAlternate(response, error_code);
      break;
    case kStunAllocationMismatch:
      ...
      // port_->loop()->Post(RTC_FROM_HERE, port_,
...
void TurnAllocateRequest::OnTimeout() {
  LOG(TRACE) << port_->ToString() << ": TURN allocate request "
...
void TurnAllocateRequest::OnAuthChallenge(StunMessage* response, int code) {
  ...
  if (code == kStunNotAuthorized && !port_->hash().empty()) {
    LOG(TRACE) << port_->ToString()
               << ": Failed to authenticate with the server "
                  "after challenge.";
    const StunErrorCodeAttribute* attr = response->GetErrorCode();
    port_->OnAllocateError(kStunNotAuthorized, attr ? attr->reason() : "");
    return;
  }
  ...
  const StunByteStringAttribute* realm_attr =
      response->GetByteString(kStunAttrRealm);
  if (!realm_attr) {
    LOG(TRACE) << port_->ToString() << ": Missing kStunAttrRealm attribute in "
  ...
  const StunByteStringAttribute* nonce_attr =
      response->GetByteString(kStunAttrNonce);
  if (!nonce_attr) {
    LOG(TRACE) << port_->ToString() << ": Missing kStunAttrNonce attribute in "
                                       "allocate unauthorized response.";
    return;
  }
...
void TurnAllocateRequest::OnTryAlternate(StunMessage* response, int code) {
  ...
  const StunAddressAttribute* alternate_server_attr =
      response->GetAddress(kStunAttrAlternateServer);
  if (!alternate_server_attr) {
    LOG(TRACE) << port_->ToString()
               << ": Missing kStunAttrAlternateServer "
                  "attribute in try alternate error response";
    port_->OnAllocateError(kStunTryAlteate,
  ...
  const StunByteStringAttribute* realm_attr =
      response->GetByteString(kStunAttrRealm);
  if (realm_attr) {
    LOG(INFO) << port_->ToString() << ": Applying kStunAttrRealm attribute in "
  ...
  const StunByteStringAttribute* nonce_attr =
      response->GetByteString(kStunAttrNonce);
  if (nonce_attr) {
    LOG(INFO) << port_->ToString() << ": Applying kStunAttrNonce attribute in "
                                      "try alternate error response.";
    port_->set_nonce(nonce_attr->GetString());
  }
...
TurnRefreshRequest::TurnRefreshRequest(TurnPort* port)
    : StunRequest(loop_, new TurnMessage()), port_(port), lifetime_(-1) {}
...
void TurnRefreshRequest::Prepare(StunMessage* request) {
  ...
  if (lifetime_ > -1) {
    request->AddAttribute(
        std::make_unique<StunUInt32Attribute>(kStunAttrLifetime, lifetime_));
  }
...
void TurnRefreshRequest::OnResponse(StunMessage* response) {
  LOG(INFO) << port_->ToString()
            << ": TURN refresh requested successfully, id=" << hex_encode(id())
            << ", code=0"  // Makes logging easier to parse.
               ", rtt=" << Elapsed();
...
void TurnRefreshRequest::OnTimeout() {
  LOG(TRACE) << port_->ToString() << ": TURN refresh timeout "
             << hex_encode(id());
  port_->OnRefreshError();
}

TurnCreatePermissionRequest::TurnCreatePermissionRequest(
    TurnPort* port, TurnEntry* entry, const SocketAddress& ext_addr,
    const std::string& remote_ufrag)
    : StunRequest(loop_, new TurnMessage()),
      port_(port),
      entry_(entry),
      ext_addr_(ext_addr),
      remote_ufrag_(remote_ufrag) {
...
void TurnCreatePermissionRequest::Prepare(StunMessage* request) {
  ...
  request->AddAttribute(std::make_unique<StunXorAddressAttribute>(
      kStunAttrXorPeerAddress, ext_addr_));
  // if (webfield_trial::IsEnabled("WebRTC-TurnAddMultiMapping")) {
...
void TurnCreatePermissionRequest::OnSent() {
  LOG(INFO) << port_->ToString()
            << ": TURN create permission request sent, id=" << hex_encode(id());
  StunRequest::OnSent();
}
...
void TurnCreatePermissionRequest::OnResponse(StunMessage* response) {
  LOG(INFO) << port_->ToString()
            ...
            << hex_encode(id()) << ", code=0"  // Makes logging easier to parse.
                                   ", rtt=" << Elapsed();
...
void TurnCreatePermissionRequest::OnTimeout() {
  LOG(TRACE) << port_->ToString() << ": TURN create permission timeout "
             << hex_encode(id());
  if (entry_) {
...
TurnChannelBindRequest::TurnChannelBindRequest(TurnPort* port, TurnEntry* entry,
                                               int channel_id,
                                               const SocketAddress& ext_addr)
    : StunRequest(loop_, new TurnMessage()),
      port_(port),
...
void TurnChannelBindRequest::Prepare(StunMessage* request) {
  ...
  request->SetType(kTurnChannelBindRequest);
  request->AddAttribute(std::make_unique<StunUInt32Attribute>(
      kStunAttrChannelNumber, channel_id_ << 16));
  request->AddAttribute(std::make_unique<StunXorAddressAttribute>(
      kStunAttrXorPeerAddress, ext_addr_));
  port_->AddRequestAuthInfo(request);
  port_->TurnCustomizerMaybeModifyOutgoingStunMessage(request);
...
void TurnChannelBindRequest::OnSent() {
  LOG(INFO) << port_->ToString()
            << ": TURN channel bind request sent, id=" << hex_encode(id());
  StunRequest::OnSent();
}
...
void TurnChannelBindRequest::OnResponse(StunMessage* response) {
  ...
  if (entry_) {
    ...
    LOG(INFO) << port_->ToString() << ": Scheduled channel bind in " << delay
              << "ms.";
  }
}
...
void TurnChannelBindRequest::OnTimeout() {
  LOG(TRACE) << port_->ToString() << ": TURN channel bind timeout "
             << hex_encode(id());
  if (entry_) {
...
int TurnEntry::Send(const void* data, size_t size, bool payload,
  ...
  if (state_ != STATE_BOUND ||
    ...
    msg.SetTransactionID(CreateRandomString(kStunTransactionIdLength));
    msg.AddAttribute(std::make_unique<StunXorAddressAttribute>(
        kStunAttrXorPeerAddress, ext_addr_));
    msg.AddAttribute(
        std::make_unique<StunByteStringAttribute>(kStunAttrData, data, size));
...
void TurnEntry::OnCreatePermissionSuccess() {
  LOG(INFO) << port_->ToString() << ": Create permission for "
  ...
  if (state_ != STATE_BOUND) {
    ...
    LOG(INFO) << port_->ToString()
              << ": Scheduled create-permission-request in " << delay << "ms.";
  }
}

void TurnEntry::OnCreatePermissionError(StunMessage* response, int code) {
  if (code == kStunStaleNonce) {
    if (port_->UpdateNonce(response)) {
      SendCreatePermissionRequest(0);
    }
  } else {
    ...
    if (found) {
      LOG(ERROR) << "Received TURN CreatePermission error response, "
...
void TurnEntry::OnChannelBindSuccess() {
  LOG(INFO) << port_->ToString() << ": Successful channel bind for "
