...
#include "stun.h"
#include "async_stun_tcp_socket.h"
...
class TurnServerAllocation::Permission : public MessageHandler {
 public:
  Permission(EventLoop* loop, const IPAddress& peer);
  ~Permission() override;

  const IPAddress& peer() const { return peer_; }
  void Refresh();

  signal1<Permission*> SignalDestroyed;
...
// allocation, and self-deletes when its lifetime timer expires.
class TurnServerAllocation::Channel : public MessageHandler {
 public:
  Channel(EventLoop* loop, int id, const SocketAddress& peer);
  ~Channel() override;
  ...
  signal1<Channel*> SignalDestroyed;
...
static bool InitErrorResponse(const StunMessage* req, int code,
  ...
  resp->AddAttribute(std::make_unique<StunErrorCodeAttribute>(
      kStunAttrStunErrorCode, code, reason));
  return true;
}

TurnServer::TurnServer(EventLoop* loop)
    : loop_(loop),
...
TurnServer::~TurnServer() {
  // assert(thread_checker_.IsCurrent());
...
void TurnServer::AddInternalServerSocket(AsyncSocket* socket,
                                         ProtocolType proto) {
  // assert(thread_checker_.IsCurrent());
  assert(server_listen_sockets_.end() == server_listen_sockets_.find(socket));
...
void TurnServer::OnNewInternalConnection(AsyncSocket* socket) {
  // assert(thread_checker_.IsCurrent());
  assert(server_listen_sockets_.find(socket) != server_listen_sockets_.end());
  AcceptConnection(socket);
}

void TurnServer::AcceptConnection(AsyncSocket* server_socket) {
  // assert(thread_checker_.IsCurrent());
  // Check if someone is trying to connect to us.
  SocketAddress accept_addr;
  AsyncSocket* accepted_socket = server_socket->Accept(&accept_addr);
  if (accepted_socket != NULL) {
    ProtocolType proto = server_listen_sockets_[server_socket];
    AsyncStunTCPSocket* tcp_socket =
        new AsyncStunTCPSocket(accepted_socket, false);
...
void TurnServer::OnInternalPacket(AsyncPacketSocket* socket, const char* data,
...
void TurnServer::HandleStunMessage(TurnServerConnection* conn, const char* data,
  ...
  if (!msg.Read(&buf) || (buf.Length() > 0)) {
    LOG(WARNING) << "Received invalid STUN message";
    return;
  }
  ...
  if (redirect_hook_ != NULL && msg.type() == kStunAllocateRequst) {
  ...
  if (!allocation && msg.type() == kStunAllocateRequst) {
    HandleAllocateRequest(conn, &msg, key);
  } else if (allocation &&
             (msg.type() != kStunAllocateRequst ||
    ...
    if (IsStunRequestType(msg.type()) &&
        msg.GetByteString(kStunAttrUsername)->GetString() !=
            allocation->username()) {
      SendErrorResponse(conn, &msg, kStunWrongCredentials,
                        StunErrorToString(kStunWrongCredentials));
      return;
    }
    allocation->HandleTurnMessage(&msg);
  } else {
    // Allocation mismatch.
    SendErrorResponse(conn, &msg, kStunAllocationMismatch,
                      StunErrorToString(kStunAllocationMismatch));
  }
}
...
bool TurnServer::GetKey(const StunMessage* msg, std::string* key) {
  ...
  const StunByteStringAttribute* username_attr =
      msg->GetByteString(kStunAttrUsername);
  if (!username_attr) {
    return false;
  }
...
bool TurnServer::CheckAuthorization(TurnServerConnection* conn,
                                    const StunMessage* msg, const char* data,
                                    size_t size, const std::string& key) {
  // assert(thread_checker_.IsCurrent());
  // RFC 5389, 10.2.2.
  assert(IsStunRequestType(msg->type()));
  const StunByteStringAttribute* mi_attr =
      msg->GetByteString(kStunAttrMessageIntegrity);
  const StunByteStringAttribute* username_attr =
      msg->GetByteString(kStunAttrUsername);
  const StunByteStringAttribute* realm_attr =
      msg->GetByteString(kStunAttrRealm);
  const StunByteStringAttribute* nonce_attr =
      msg->GetByteString(kStunAttrNonce);

  // Fail if no M-I.
  if (!mi_attr) {
    SendErrorResponseWithRealmAndNonce(conn, msg, kStunNotAuthorized,
                                       StunErrorToString(kStunNotAuthorized));
    return false;
  }

  // Fail if there is M-I but no username, nonce, or realm.
  if (!username_attr || !realm_attr || !nonce_attr) {
    SendErrorResponse(conn, msg, kStunBadRequest,
  ...
  if (!ValidateNonce(nonce_attr->GetString())) {
    SendErrorResponseWithRealmAndNonce(conn, msg, kStunStaleNonce,
                                       StunErrorToString(kStunStaleNonce));
    return false;
  }
...
void TurnServer::HandleBindingRequest(TurnServerConnection* conn,
  ...
  auto mapped_addr_attr = std::make_unique<StunXorAddressAttribute>(
      kStunAttrXorMappedAddress, conn->src());
...
void TurnServer::HandleAllocateRequest(TurnServerConnection* conn,
  ...
  const StunUInt32Attribute* transport_attr =
      msg->GetUInt32(kStunAttrRequestedTransport);
  if (!transport_attr) {
    SendErrorResponse(conn, msg, kStunBadRequest,
                      StunErrorToString(kStunBadRequest));
    return;
  ...
  if (proto != IPPROTO_UDP) {
    SendErrorResponse(conn, msg, kStunUnsuppTransportProto,
                      StunErrorToString(kStunUnsuppTransportProto));
    return;
  }
  ...
  if (alloc) {
    ...
    SendErrorResponse(conn, msg, kStunServerError, "Failed to allocate socket");
  }
}
...
std::string TurnServer::GenerateNonce(int64_t now) const {
  ...
  std::string nonce = hex_encode(input.c_str(), input.size());
...
bool TurnServer::ValidateNonce(const std::string& nonce) const {
  ...
  size_t len = hex_decode(p, sizeof(then), nonce.substr(0, sizeof(then) * 2));
  if (len != sizeof(then)) {
    return false;
  }
  ...
  return TimeMillis() - then < kNonceTimeout;
}
...
TurnServerAllocation* TurnServer::CreateAllocation(TurnServerConnection* conn,
  ...
  TurnServerAllocation* allocation =
      new TurnServerAllocation(this, loop_, *conn, external_socket, key);
...
void TurnServer::SendErrorResponse(TurnServerConnection* conn,
  ...
  LOG(INFO) << "Sending error response, type=" << resp.type()
...
void TurnServer::SendErrorResponseWithRealmAndNonce(TurnServerConnection* conn,
  ...
  resp.AddAttribute(std::make_unique<StunByteStringAttribute>(
      kStunAttrNonce, GenerateNonce(timestamp)));
  resp.AddAttribute(
...
void TurnServer::SendErrorResponseWithAlternateServer(
  ...
  // assert(thread_checker_.IsCurrent());
  TurnMessage resp;
  InitErrorResponse(msg, kStunTryAlteate, StunErrorToString(kStunTryAlteate),
                    &resp);
  resp.AddAttribute(
      std::make_unique<StunAddressAttribute>(kStunAttrAlternateServer, addr));
  SendStun(conn, &resp);
}
...
void TurnServer::DestroyInternalSocket(AsyncPacketSocket* socket) {
  ...
  if (iter != server_sockets_.end()) {
    ...
    sockets_to_delete_.push_back(std::unique_ptr<AsyncPacketSocket>(socket));
    // invoker_.AsyncInvoke<void>(RTC_FROM_HERE, Thread::Current(),
    //                            Bind(&TurnServer::FreeSockets, this));
  }
}
...
TurnServerAllocation::TurnServerAllocation(TurnServer* server, EventLoop* loop,
...
TurnServerAllocation::~TurnServerAllocation() {
  ...
  LOG(INFO) << ToString() << ": Allocation destroyed";
}
...
void TurnServerAllocation::HandleAllocateRequest(const TurnMessage* msg) {
  ...
  const StunByteStringAttribute* origin_attr =
      msg->GetByteString(kStunAttrOrigin);
  if (origin_attr) {
  ...
  auto mapped_addr_attr = std::make_unique<StunXorAddressAttribute>(
      kStunAttrXorMappedAddress, conn_.src());
  auto relayed_addr_attr = std::make_unique<StunXorAddressAttribute>(
      kStunAttrXorRelayedAddress, external_socket_->GetLocalAddress());
  auto lifetime_attr =
      std::make_unique<StunUInt32Attribute>(kStunAttrLifetime, lifetime_secs);
  response.AddAttribute(std::move(mapped_addr_attr));
...
void TurnServerAllocation::HandleSendIndication(const TurnMessage* msg) {
  ...
  if (!data_attr || !peer_attr) {
    LOG(WARNING) << ToString() << ": Received invalid send indication";
    return;
  }
...
void TurnServerAllocation::HandleCreatePermissionRequest(
  ...
  if (server_->reject_private_addresses_ &&
    ...
    SendErrorResponse(msg, kStunForbidden, StunErrorToString(kStunForbidden));
    return;
  }
  ...
  LOG(INFO) << ToString() << ": Created permission, peer="
...
void TurnServerAllocation::HandleChannelBindRequest(const TurnMessage* msg) {
  ...
  const StunUInt32Attribute* channel_attr =
      msg->GetUInt32(kStunAttrChannelNumber);
  const StunAddressAttribute* peer_attr =
      msg->GetAddress(kStunAttrXorPeerAddress);
  ...
  if (!channel1) {
    channel1 = new Channel(loop_, channel_id, peer_attr->GetAddress());
  ...
  LOG(INFO) << ToString() << ": Bound channel, id=" << channel_id
            << ", peer=" << peer_attr->GetAddress().ToSensitiveString();
...
void TurnServerAllocation::HandleChannelData(const char* data, size_t size) {
  ...
  uint16_t channel_id = GetBE16(data);
...
void TurnServerAllocation::OnExternalPacket(
    AsyncPacketSocket* socket, const char* data, size_t size,
    const SocketAddress& addr, const int64_t& /* packet_time_us */) {
...
int TurnServerAllocation::ComputeLifetime(const TurnMessage* msg) {
  ...
  const StunUInt32Attribute* lifetime_attr = msg->GetUInt32(kStunAttrLifetime);
...
void TurnServerAllocation::AddPermission(const IPAddress& addr) {
  ...
  if (!perm) {
    perm = new Permission(loop_, addr);
...
TurnServerAllocation::Permission* TurnServerAllocation::FindPermission(
  ...
  for (PermissionList::const_iterator it = perms_.begin(); it != perms_.end();
       ++it) {
    if ((*it)->peer() == addr) return *it;
  }
  return NULL;
}

TurnServerAllocation::Channel* TurnServerAllocation::FindChannel(int channel_id)
    const {
  for (ChannelList::const_iterator it = channels_.begin();
       it != channels_.end(); ++it) {
    if ((*it)->id() == channel_id) return *it;
  }
  return NULL;
}
...
void TurnServerAllocation::SendBadRequestResponse(const TurnMessage* req) {
  SendErrorResponse(req, kStunBadRequest, StunErrorToString(kStunBadRequest));
}
...
void TurnServerAllocation::OnMessage(Message* msg) {
...
TurnServerAllocation::Permission::Permission(EventLoop* loop,
...
void TurnServerAllocation::Permission::OnMessage(Message* msg) {
  assert(msg->message_id == MSG_ALLOCATION_TIMEOUT);
  SignalDestroyed(this);
  delete this;
}

TurnServerAllocation::Channel::Channel(EventLoop* loop, int id,
                                       const SocketAddress& peer)
    : loop_(loop), id_(id), peer_(peer) {
  Refresh();
}
...
void TurnServerAllocation::Channel::OnMessage(Message* msg) {
  assert(msg->message_id == MSG_ALLOCATION_TIMEOUT);
  SignalDestroyed(this);
  delete this;
}
