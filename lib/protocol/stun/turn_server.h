...
class TurnServerConnection {
 public:
  ...
  TurnServerConnection(const SocketAddress& src, ProtocolType proto,
...
class TurnServerAllocation : public MessageHandler, public has_slots<> {
 public:
  TurnServerAllocation(TurnServer* server, EventLoop* loop,
  ...
  signal1<TurnServerAllocation*> SignalDestroyed;
 ...
 private:
  ...
  void OnExternalPacket(AsyncPacketSocket* socket, const char* data,
  ...
  Permission* FindPermission(const IPAddress& addr) const;
  Channel* FindChannel(int channel_id) const;
  Channel* FindChannel(const SocketAddress& addr) const;
...
class TurnServer : public has_slots<> {
 public:
  ...
  // will be added.
  void AddInternalServerSocket(AsyncSocket* socket, ProtocolType proto);
 ...
 private:
  ...
  void OnInternalPacket(AsyncPacketSocket* socket, const char* data,
                        size_t size, const SocketAddress& address,
                        const int64_t& packet_time_us);

  void OnNewInternalConnection(AsyncSocket* socket);

  // Accept connections on this server socket.
  void AcceptConnection(AsyncSocket* server_socket);
  void OnInternalSocketClose(AsyncPacketSocket* socket, int err);

  void HandleStunMessage(TurnServerConnection* conn, const char* data,
                         size_t size);
  ...
  TurnServerAllocation* CreateAllocation(TurnServerConnection* conn, int proto,
                                         const std::string& key);
  ...
  std::vector<std::unique_ptr<AsyncPacketSocket>> sockets_to_delete_;
  std::unique_ptr<PacketSocketFactory> external_socket_factory_;
  SocketAddress external_addr_;
