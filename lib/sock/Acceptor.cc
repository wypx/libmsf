...
Acceptor::Acceptor(EventLoop *loop, const InetAddress & addr, const NewConnCb & cb)
  : quit_(false),
    loop_(loop),
    addr_(addr),
    newConnCb_(std::move(cb))
{
  if (!listen()) {
    MSF_FATAL << "Fail to listen for " << addr.hostPort2String();
  }
  openIdleFd();
}
...
void Acceptor::openIdleFd()
{
  ...
  if (idleFd_ < 0) {
    MSF_WARN << "Open idle fd failed, errno: " << errno;
  }
}
...
bool Acceptor::enableListen()
{
  if (::listen(sfd_, backLog_) < 0) {
    MSF_ERROR << "Failed to enable listen for fd: " << sfd_;
    return false;
  }
  return true;
}
...
{
  if (::listen(sfd_, 0) < 0) {
    MSF_ERROR << "Failed to disable listen for fd: " << sfd_;
    return false;
  }
  return true;
}
...
bool Acceptor::listen()
{
  ...
  if (addr_.family() == AF_INET || addr_.family() == AF_INET6) {
    if (addr_.proto() == SOCK_STREAM) {
      MSF_INFO << "host: " << addr_.host() << " port: " << addr_.port();
    ...
    MSF_WARN << "Not support this family type: " << addr_.family();
    return false;
  }
      ...
      if (newFd < 0) {
          ...
          if (errno == EAGAIN || errno == EWOULDBLOCK || errno == ECONNABORTED) {
            ...
            //系统的上限ENFILE 进程的上限EMFILE
              MSF_ERROR << "Too many open connections, give up accept.";
...
{
  MSF_INFO << "Acceptor close for fd: " << sfd_;
  if (sfd_ > 0) {
    close(sfd_);
    sfd_ = -1;
  }
