...
InetAddress::InetAddress() : 
    _host(DEFAULT_IPANY), _port(DEFAULT_PORT), _family(AF_INET), _type(0), _proto(SOCK_STREAM)
{
}
...
{ 
    ...
    if (inet_pton(AF_INET, host.c_str(), &addr4)) {
        ...
        if (host.size() > sizeof(_addrun.sun_path)) {
            MSF_ERROR << "Unix path: " << host << " is too long : " << host.size();
        } else {
...
bool InetAddress::sockAddrCmp(const struct sockaddr *sa1, const struct sockaddr *sa2)
{
