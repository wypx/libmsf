...
void DebugSocket(const int fd)
{
    ...
    if (cliAddr.ss_family == AF_UNIX) { 
        ...
        MSF_INFO << "new conn from ipv4: " << ip << ", port: " << ntohs(sin->sin_port);
    } else if (cliAddr.ss_family == AF_INET6) {
        sin6 = SIN6(&cliAddr);
        inet_ntop(cliAddr.ss_family, &sin6->sin6_addr, ip, sizeof(ip));
        MSF_INFO << "new conn from ipv6: " << ip << ", port: " << ntohs(sin6->sin6_port);
    }
...
bool SetReuseAddr(const int fd, bool on)
{
    ...
    * processes from binding to this address while we're using it. */
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (void*)&one, (int)sizeof(one)) < 0) {
        return false;
    }
    #endif
    return true;
...
bool SetIpv6Only(const int fd, bool on)
{
    #ifdef IPV6_V6ONLY
    int only = on;
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&only, sizeof(only)) < 0) {
...
static int SocketTcpNagleOffv1(int fd) MSF_ATTRIBUTE_WIKEREF_ALIAS(SocketTcpNodelay);
...
{
    MSF_INFO  << "Socket, set Cork Flag FD " << fd
              << "to" << (on ? "ON" : "OFF");
...
bool SetTcpDeferAccept(const int fd)
{
    ...
    * has arrived and ready to read */
    ...
    int timeout = 0;
    if (setsockopt(fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &timeout, sizeof(int)) < 0) {
        return false;
    }
    #else
...
int BindAddress(const int fd, const struct sockaddr *addr, const socklen_t addrlen)
{
    int ret = -1;
...
bool dropMuticalastMemershipv4(const int fd, const std::string &ip)
{
    ...
    // mreq.imr_interface.s_addr = ((struct sockaddr_in *)&mreq.ifr_addr)->sin_addr.s_addr;
    mreq.imr_multiaddr.s_addr = inet_addr(ip.c_str());
    mreq.imr_interface.s_addr = htonl(INADDR_ANY);
...
bool dropMuticalastMemershipv6(const int fd, const std::string &ip)
{
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IP_DROP_MEMBERSHIP, &merq6, sizeof(merq6)) < 0) {
        MSF_ERROR << "Setsockopt Error:IP_DROP_MEMBERSHIP, errno:" << errno;
        return false;
    }
    memcpy(&merq6.ipv6mr_multiaddr, &ipv6.sin6_addr, sizeof(struct in6_addr));
    // merq6.ipv6mr_interface = if_nametoindex(merq6.ifr_name);
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_DROP_MEMBERSHIP, &merq6, sizeof(merq6)) < 0) {
...
int CreateUnixServer(const std::string & srvPath, const uint32_t backlog, const uint32_t access_mask) 
{
    ...
    */
    ...
    memcpy(s_un.sun_path, srvPath.c_str(), std::min(sizeof(s_un.sun_path)-1, srvPath.size()));
    ...
    if (bind(fd, (struct sockaddr *)&s_un, sizeof(s_un)) < 0) {
        umask(old_umask);
        MSF_ERROR << "Bind failed  for " << s_un.sun_path << "errno: " << strerror(errno);
        close(fd);
        return -1;
    }
    umask(old_umask);
