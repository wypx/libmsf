...
*
**************************************************************************/
#include <base/mongodb/msf_zookeeper.h>

using namespace MSF::ZOOKEEPER;
...
void UZookeeperBase::InitWatcher(zhandle_t *zh, int type, int state, const char *path, void *watcher_ctx)
{
...
int UZookeeperBase::ConnectZookeeper()
{
    ...
    do {
        pstZkHandle = zookeeper_init(getHostPorts(), InitWatcher, iConnectTimeout, NULL, NULL, 0);
    ...
    if (iConnTimes >= iMaxReConnectCnt) {
        cout<<"Connect zk host " << getHostPorts() << " over max times:" << iConnTimes <<endl;
        return -1;
    }
...
UZookeeperSrv::UZookeeperSrv(/* args */)
{
    pstZkSrv = this;
}


void UZookeeperSrv::CreateParentComplete(int rc, const char* value, const void* data)
 {
    cout<<"CreateParentComplete --> zoo_create() path=" << value << "," << zerror(rc) <<endl;
    switch (rc) {
        //判断返回码来确定需要如何处理
        case ZCONNECTIONLOSS:
            cout<<"CreateParentComplete: Try again later."<<endl;
            pstZkSrv->CreateParent(value, (const char*)data);//在连接丢失时进行重试操作。
            break;
        case ZOK:
        ...
        case ZNODEEXISTS:
            ...
            break;
        default:
            cout<<"CreateParentComplete: Something went wrong when running for master."<<endl;
            break;
  }
}
...
* zoo_create 为同步调用
* 参数1: zkhandle
* 参数2: 该方法的path参数类型为const char*类型,path用于将客户端与对应的znode节点的子树连接在一起
* 参数3: 该函数的第三个参数为存储到znode节点的数据信息
* 参数4: 该参数为保存数据信息（前一个参数）的长度值,本例中设置为0
* 参数5: 本例中，我们并不关心ACL策略问题,所以我们均设置为unsafe模式
* 参数6: 根结点些znode节点为持久性的非有序节点,所以我们不需要传入任何标志位
* 参数7: 异步调用,我们需要传入一个完成函数,ZooKeeper客户端会在操作请求完成时调用该函数
*        同步调用,
* 参数8: 异步调用，最后一个参数为上下文变量, 本例中不需要传入任何上下文变量
*        同步调用,
* */
void UZookeeperSrv::CreateZnode(const char* path, const char* value, int valuelen, 
                            int flags, string_completion_t completion)
{
    zoo_acreate(pstZkHandle, path, value, valuelen,
                &ZOO_OPEN_ACL_UNSAFE, flags, completion, NULL);
...
void UZookeeperSrv::MasterExistWatcher(zhandle_t* zh, int type, int state, const char* path, void* watcherCtx) 
{
...
void UZookeeperSrv::MasterExistComplete(int rc, const struct Stat* stat, const void* data) 
{
  switch (rc) {
    case ZCONNECTIONLOSS:
...
void UZookeeperSrv::CreateMasterComplete(int rc, const char* value, const void* data) 
{
    switch (rc) {
        case ZCONNECTIONLOSS:
        ...
        case ZNODEEXISTS:
            ...
            //如果主节点znode节点存在(其他进程已经创建并锁定该节点), 就对该节点建立监视点, 来监视该节点之后可能消失的情况。
            pstZkSrv->MasterExist();
            break;
        default:
            cout<<"CreateMasterComplete: Something went wrong when running for master."<<endl;
            break;
    }
}
...
void UZookeeperSrv::CreateMaster() 
{
    ...
    CreateZnode("/master", (const char*)server_id_string, sizeof(int), ZOO_EPHEMERAL, CreateMasterComplete);
}


void UZookeeperSrv::MakeLeaderShip()
{
    GetWorkers();
}
...
void UZookeeperSrv::GetWorkersComplete(int rc, const struct String_vector* strings, const void* data) 
{

}


void UZookeeperSrv::CreateUDiskIpGroupComplete(int rc, const char* value, const void* data) 
{
    switch (rc) {
        //判断返回码来确定需要如何处理
        case ZCONNECTIONLOSS:
        ...
        case ZNODEEXISTS:
            ...
            break;
        default:
            cout<<"CreateUDiskIpGroupComplete: Something wrong with master, "<<zerror(rc)<<"."<<endl;
            break;
  }
}
...
void UZookeeperSrv::CreateUDiskIpItemComplete(int rc, const char* value, const void* data) 
{
    cout<<"CreateUDiskIpItemComplete --> zoo_create() path=" << value << ",ret:" << zerror(rc) <<endl;
    switch (rc) {
        //判断返回码来确定需要如何处理
        case ZCONNECTIONLOSS:
        ...
        case ZOK:
            ...
            break;
        case ZNODEEXISTS:
            cout<<"CreateUDiskIpItemComplete: Something wrong with master, "<<zerror(rc)<<"."<<endl;
            break;
        default:
            cout<<"CreateUDiskIpItemComplete: Something went wrong when running for master."<<endl;
            break;
  }
}
void UZookeeperSrv::CreateUDiskIpItem() 
{
    ...
    CreateZnode(pstUDiskIpItemPath.c_str(), pstUDiskIpItemPath.c_str(), pstUDiskIpItemPath.length(),
                 ZOO_EPHEMERAL, CreateUDiskIpItemComplete);
}
...
UZookeeperCli::UZookeeperCli(/* args */)
{
    pstZkCli = this;
}

void UZookeeperCli::UpdateUDiskIpGroupWatcher(zhandle_t *zh, int type, int state, const char *path, void *watcherCtx)
{
    cout << "######################################" << endl;
    cout << "type:" << type << endl;
...
void UZookeeperCli::UpdateUDiskIpGroup(/* args */)
{
    ...
    int iRet = zoo_wget_children(pstZkHandle, pstUDiskIpGroup.c_str(), UpdateUDiskIpGroupWatcher, NULL, &str_vec);
    if (iRet) {
        cout << "UpdateUDiskIpGroup --> read path:" << pstUDiskIpGroup << " wrong, " << zerror(iRet) << endl;
        return;
    }

    cout << "UpdateUDiskIpGroup --> path:" << pstUDiskIpGroup << ", ret:" << iRet << ", node's size:" << str_vec.count << endl;

    //获得各份服务ip:port
    for (int i = 0; i < str_vec.count; ++i) {
        struct String_vector node_vec;
        string path = pstUDiskIpGroup + "/" + str_vec.data[i];
        iRet = zoo_wget_children(pstZkHandle, path.c_str(), UpdateUDiskIpGroupWatcher, NULL, &node_vec);
        cout << "UpdateUDiskIpGroup --> path:" << path << ", ret:" << iRet << ", node's size:" << node_vec.count << endl;
