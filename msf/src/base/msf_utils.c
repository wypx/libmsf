...
u8 safe_strtoull(const s8 *str, u64 *out) {
    ...
    *out = 0;
    ...
    if (msf_isspace(*endptr) || (*endptr == '\0' && endptr != str)) {
        if ((long long) ull < 0) {
            /* only check for negative signs in the uncommon case when
             * the unsigned number is so big that it's negative as a
             * signed number. */
            if (msf_strchr(str, '-') != NULL) {
                return MSF_FALSE;
            }
        }
        *out = ull;
...
void write_stacktrace(const s8 *file_name) {
    s32 fd = open(file_name, O_WRONLY | O_APPEND | O_CREAT, 0644);
    void *buffer[100];
    s32 nptrs;
