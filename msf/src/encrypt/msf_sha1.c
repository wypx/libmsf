...
static inline u32 rol32(u32 word, u32 shift)
{
	return (word << shift) | (word >> (32 - shift));
}

static void __sha1_transform(u32 hash[SHA1_DIGEST_WORDS],
        const u32 in[SHA1_BLOCK_WORDS],  u32 W[SHA1_WORKSPACE_WORDS]) {
...
void sha1_final(struct sha1_ctx *ctx, u8 *out) {
    const size_t offset = ctx->count & 0x3f;
    s8 *p = (char *)ctx->block + offset;
    s32 padding = (SHA1_BLOCK_BYTES - SHA1_COUNTER_BYTES) - (offset + 1);
    s32 i;
