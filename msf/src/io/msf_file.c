...
s32 msf_open_tempfile(u8 *name, u32 persistent, u32 access)
{
    ...
    if (fd != MSF_INVALID_SOCKET && !persistent) {
...
s32 msf_create_paths(s8 *path, uid_t user) {
    ...
    if (msf_stat((const s8 *) path, &fi) == -1) {
        return -1;
    }

    if (fi.st_uid != user) {
        if (chown((const s8 *) path, user, -1) == -1) {
            return -1;
        }
    }
...
s32 msf_get_file_info(const s8 *path, struct msf_file *file, u32 mode) {
    ...
    file->is_exist = MSF_TRUE;
    file->is_file = MSF_TRUE;
    file->is_link = MSF_FALSE;
    file->is_dir = MSF_FALSE;
    file->exec_access = MSF_FALSE;
    file->read_access = MSF_FALSE;

    if (S_ISLNK(f.st_mode)) {
        file->is_link = MSF_TRUE;
        file->is_file = MSF_FALSE;
        if (stat(path, &target) == MSF_ERR) {
...
s32 msf_chk_file_exist(const s8 *file) {
    return (access(file, R_OK | W_OK) >= 0) ? MSF_OK : MSF_ERR;
}
