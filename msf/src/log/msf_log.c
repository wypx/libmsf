...
s32 msf_log_init(const s8 *log_path) {
    ...
    lplog->enable       = MSF_TRUE;
    lplog->blogout      = MSF_TRUE;
    lplog->blogfile     = MSF_TRUE;
    lplog->blogredirect = MSF_TRUE;

    lplog->bzip         = MSF_TRUE;
    lplog->zipalg       = MSF_FALSE;
    lplog->bencrypt     = MSF_FALSE;
    lplog->encryptalg   = MSF_FALSE;
    lplog->bupload      = MSF_FALSE;
    lplog->bdownload    = MSF_FALSE;

    lplog->logfd        = MSF_INVALID_SOCKET;
