    ...
    socklen_t addrlen, u32 backlog)
{
    ...
    switch (e) {
        ...
        case MSF_EADDRINUSE:
            ...
            break;
        case MSF_EADDRNOTAVAIL:
            MSF_NETWORK_LOG(DBG_ERROR, "IP addr maybe has been changed.");
            break;
        default:
    ...
    */
    ...
    if (kernel_features & KERNEL_TCP_FASTOPEN) {
        ...
        if (unlikely(ret == MSF_ERR)) {
            MSF_NETWORK_LOG(DBG_ERROR, "Could not set TCP_FASTOPEN.");
        }
    }

    ret = msf_socket_listen(fd, backlog);
    if (unlikely(ret < 0)) {
        return MSF_ERR;
    }
...
void msf_drain_fd(s32 fd, u32 n_packets) {
    ...
    if (MSF_INVALID_SOCKET == fd) return;
...
s32 msf_server_unix_socket(s32 backlog, s8 *unixpath, s32 access_mask) {
    ...
    */
    ...
    if (msf_socket_listen(sfd, backlog) < 0) { 
...
s32 msf_server_socket(s8 *interf, s32 protocol, s32 port, s32 backlog) {
    ...
    if (bind(sfd, next->ai_addr, next->ai_addrlen) == -1) {
        ...
        if (!IS_UDP(protocol) &&
            msf_socket_listen(sfd, backlog) < 0) {
        perror("listen()");
        goto err;
        }
