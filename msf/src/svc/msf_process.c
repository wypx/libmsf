...
pid_t msf_getpid_by_name(s8 *name)
{
    FILE *fptr;
    s8 buf[256] = { 0 };
    s8 cmd[256] = { 0 };
    pid_t pid = -1;
    msf_snprintf(cmd, sizeof(cmd) -1, "pidof %s", name);
...
s32 msf_get_process_name(s8 *name)
{
    FILE *fptr;
    s8 cmd[256] = { 0 };
    
    msf_snprintf(cmd, sizeof(cmd) - 1,
            "readlink /proc/%d/exe", msf_getpid());
    if ((fptr = popen(cmd,"r")) != NULL) {
