...
s32 foo() __attribute__ ((weak, alias("__foo")));
//等效  asm(".weak foo\n\t .set symbol1, __foo\n\t");
...
s32 msf_svc_stub(const s8 *func, ...)
{
    ...
    MSF_SVC_LOG(DBG_DEBUG, "Func(%s) not found here.", func);
    return MSF_SVC_NOT_FOUND;
}
...
s32 msf_svcinst_init(struct svcinst *svc) {
    ...
    if (!svc->svc_cb) {
        ...
        MSF_SVC_LOG(DBG_DEBUG, "Svc load: '%s' successful.", svc->svc_lib);
    }
...
s32 msf_insmod(const s8 *path)
{
    ...
    fd = msf_open_file(path, MSF_FILE_RDONLY, MSF_FILE_TRUNCATE,
                    MSF_FILE_DEFAULT_ACCESS);
    if (fd < 0) {
        MSF_SVC_LOG(DBG_ERROR, "Open %s failed: 0x%x.", path, errno);
        return MSF_ERR;
    }
    ...
    if (rc < 0) {
        sclose(fd);
        MSF_SVC_LOG(DBG_ERROR, "Fstat %s failed: 0x%x.", path, errno);
        return MSF_ERR;
    }

    len = st.st_size;
    map = MSF_MMAP(NULL, len, PROT_READ, MAP_PRIVATE, fd, 0);
    if (map == MSF_MMAP_FAILED) {
        sclose(fd);
        MSF_SVC_LOG(DBG_ERROR, "Cannot mmap %s: 0x%x.", path, errno);
        return MSF_ERR;
    }
    ...
    if (rc != 0)
    {
        MSF_MUNMAP(map, len);
        sclose(fd);
        MSF_SVC_LOG(DBG_ERROR, "cannot insert %s: 0x%x (%li).", path, errno, rc);
        return MSF_ERR;
    }
    MSF_MUNMAP(map, len);
    sclose(fd);
...
s32 msf_rmmod(const s8 *path, s32 mode)
{
    ...
    iFileLen = strlen(path);
    if (iFileLen >= 128 || iFileLen == 0) {
        MSF_SVC_LOG(DBG_ERROR, "filename length error: %d.", iFileLen);
        return MSF_ERR;
    }
    ...
    lRet = syscall(__NR_delete_module, path, flags);
    if (lRet != 0) {
        MSF_SVC_LOG(DBG_ERROR, "Cannot rmmod %s: 0x%x.", path, errno);
        return MSF_ERR;
    }
    return MSF_OK;
}
